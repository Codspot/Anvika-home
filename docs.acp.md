...
This document describes how the Anvika ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`anvika acp` exposes an ACP agent over stdio and forwards prompts to a running
Anvika Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `anvika acp` and speaks ACP messages over stdio.
...
`anvika acp` resolves the Gateway URL and auth from CLI flags or config:
