  ...
  for (const [tabId, tab] of tabs.entries()) {
    if (tab.state === 'connected') {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'Anvika Browser Relay: relay reconnecting…',
      })
    }
  }
...
async function reannounceAttachedTabs() {
  for (const [tabId, tab] of tabs.entries()) {
    ...
    try {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'Anvika Browser Relay (click to attach/detach)',
      })
      continue
    }
    ...
    try {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'Anvika Browser Relay: attached (click to detach)',
      })
    } catch {
      setBadge(tabId, 'on')
    }
...
async function connectOrToggleForActiveTab() {
  ...
  try {
    ...
    void chrome.action.setTitle({
      tabId,
      title: 'Anvika Browser Relay: connecting to local relay…',
    })
    ...
    try {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'Anvika Browser Relay: relay not running (open options for setup)',
      })
...
async function onDebuggerDetach(source, reason) {
  ...
  void chrome.action.setTitle({
    tabId,
    title: 'Anvika Browser Relay: re-attaching after navigation…',
  })
  ...
  for (let attempt = 0; attempt < delays.length; attempt++) {
    ...
    if (!relayWs || relayWs.readyState !== WebSocket.OPEN) {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'Anvika Browser Relay: relay disconnected during re-attach',
      })
      return
    }
  ...
  void chrome.action.setTitle({
    tabId,
    title: 'Anvika Browser Relay: re-attach failed (click to retry)',
  })
}
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  const { url, token } = msg
  const headers = token ? { 'x-anvika-relay-token': token } : {}
