...
- Publish as `anvika/plugin-sdk` (or export from core under `anvika/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `AnvikaPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: AnvikaConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: AnvikaConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: AnvikaConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: AnvikaConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: AnvikaConfig): string;
  };
};
```
...
- Introduce `anvika/plugin-sdk`.
- Add `api.runtime` to `AnvikaPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `anvikaRuntime: ">=2026.2.0"`).
