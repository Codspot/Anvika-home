...
description: "Complete field-by-field reference for ~/.anvika/anvika.json"
summary: "Complete reference for every Anvika config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.anvika/anvika.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Anvika uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `anvika doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["anvika-dm"] },
      guilds: {
        "123456789012345678": {
...
- Anvika additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `anvika plugins install @anvika/mattermost`.
...
Anvika spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `anvika channels add` (or channel onboarding) while still on a single-account top-level channel config, Anvika moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `anvika doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@anvika"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `anvika.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Anvika auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Anvika assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "anvika-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.anvika/sandboxes`
...
noVNC observer access uses VNC auth by default and Anvika emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `anvika-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.anvika/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@anvika"] },
        sandbox: { mode: "off" },
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Anvika starts a fresh thread session instead of inheriting parent transcript history.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:anvika`   | All built-in tools (excludes provider plugins)                                           |
...
Anvika uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.anvika/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `ANVIKA_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `anvika onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `anvika onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `anvika onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `anvika onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `anvika onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `anvika onboard --auth-choice minimax-api`.
...
- Loaded from `~/.anvika/extensions`, `<workspace>/.anvika/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `anvika plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or ANVIKA_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `ANVIKA_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.anvika-dev` + port `19001`), `--profile <name>` (uses `~/.anvika-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.anvika/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-anvika-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `ANVIKA_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or ANVIKA_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__anvika__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__anvika__/a2ui/`
...
- Also serves A2UI at `/__anvika__/a2ui/`.
...
- Hostname defaults to `anvika`. Override with `ANVIKA_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.anvika/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `anvika dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.anvika/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${ANVIKA_GATEWAY_TOKEN}" },
  },
}
```
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.anvika/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/anvika-vault-resolver",
...
- Legacy OAuth imports from `~/.anvika/credentials/oauth.json`.
...
- Default log file: `/tmp/anvika/anvika-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `anvika doctor --fix` can strip unknown keys).
