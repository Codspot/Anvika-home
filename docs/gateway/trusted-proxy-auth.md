...
read_when:
  - Running Anvika behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Anvika
...
- You run Anvika behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Anvika checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Anvika extracts the user identity from the configured header
...
- Anvika can stay on loopback HTTP behind the proxy.
...
If Anvika itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://anvika.example.com
    to: http://anvika-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`anvika security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Anvika config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `anvika security audit` and review findings
