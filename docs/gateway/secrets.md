...
Anvika supports additive secret references so credentials do not need to be stored as plaintext in config files.
...
When onboarding runs in interactive mode and you choose secret reference storage, Anvika performs a fast preflight check before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Anvika validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Anvika QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/anvika"],
...
When reload-time activation fails after a healthy state, Anvika enters degraded secrets state.
...
```bash
anvika secrets audit --check
anvika secrets configure
anvika secrets audit --check
```
...
- plaintext values at rest (`anvika.json`, `auth-profiles.json`, `.env`)
- unresolved refs
...
- lets you select secret-bearing fields in `anvika.json`
...
- `anvika secrets configure --providers-only`
- `anvika secrets configure --skip-provider-setup`
...
```bash
anvika secrets apply --from /tmp/anvika-secrets-plan.json
anvika secrets apply --from /tmp/anvika-secrets-plan.json --dry-run
```
...
Anvika intentionally does **not** write rollback backups that contain pre-migration plaintext secret values.
...
For static credentials, Anvika runtime no longer depends on plaintext `auth.json`.
