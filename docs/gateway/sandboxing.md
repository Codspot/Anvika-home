---
summary: "How Anvika sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
Anvika can run **tools inside Docker containers** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`anvika-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; Anvika emits a short-lived token URL that resolves to the observer session.
...
- `"none"` (default): tools see a sandbox workspace under `~/.anvika/sandboxes`.
- `"ro"`: mounts the agent workspace read-only at `/agent` (disables `write`/`edit`/`apply_patch`).
- `"rw"`: mounts the agent workspace read/write at `/workspace`.

Inbound media is copied into the active sandbox workspace (`media/inbound/*`).
Skills note: the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`,
Anvika mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- Anvika blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
Default image: `anvika-sandbox:bookworm-slim`
...
- Use `anvika sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
