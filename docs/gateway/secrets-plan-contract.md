...
read_when:
  - Generating or reviewing `anvika secrets apply` plan files
...
This page defines the strict contract enforced by `anvika secrets apply`.

If a target does not match these rules, apply fails before mutating config.

## Plan file shape

`anvika secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
If apply fails with an invalid target path message, regenerate the plan with `anvika secrets configure` or fix the target path to one of the allowed shapes above.
