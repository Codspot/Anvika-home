...
- During heartbeat runs, Anvika treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `anvika sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Anvika skips the heartbeat run entirely (no model call).
...
```bash
anvika system event --text "Check for urgent follow-ups" --mode now
```
