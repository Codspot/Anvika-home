...
Deploy Anvika on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/anvika/anvika)
...
repository configures everything needed to run Anvika:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: ANVIKA_STATE_DIR
        value: /data/.anvika
      - key: ANVIKA_WORKSPACE_DIR
        value: /data/workspace
      - key: ANVIKA_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/anvika`.
...
If you use the original Anvika repository, Render will not auto-deploy your Anvika. To update it, run a manual Blueprint sync from the dashboard.
...
For Anvika, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Anvika host.
