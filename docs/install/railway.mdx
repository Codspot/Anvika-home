...
Deploy Anvika on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/anvika` — Control UI
...
- `ANVIKA_STATE_DIR=/data/.anvika` (recommended)
- `ANVIKA_WORKSPACE_DIR=/data/workspace` (recommended)
- `ANVIKA_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Anvika state + workspace so you can migrate to another host without losing config or memory.
