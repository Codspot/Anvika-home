...
This plan defines how Anvika should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Anvika core owns ACP control plane concerns
...
Anvika should not reimplement ACP transport internals in core.
Anvika should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Anvika, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Anvika state dir
...
- ACP event stream is normalized to Anvika reply chunks
...
For the first production backend (`extensions/acpx`), Anvika and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Anvika `sessionKey`)
...
- Anvika consumes ndjson events from `acpx --format json --json-strict`
