...
read_when:
  - Designing workspace memory (~/.anvika/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Anvika integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.anvika/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.anvika/workspace` as canonical human-readable memory.
...
```
~/.anvika/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Anvika**, but keep a separable core library.

### Why integrate into Anvika?

- Anvika already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `anvika memory recall "…" --k 25 --since 30d`
  - `anvika memory reflect --since 7d`
...
Pragmatic take for `~/.anvika/workspace`:
