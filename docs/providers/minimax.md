---
summary: "Use MiniMax M2.1 in Anvika"
read_when:
  - You want MiniMax models in Anvika
...
```bash
anvika plugins enable minimax-portal-auth  # skip if already loaded.
anvika gateway restart  # restart if gateway is already running
anvika onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/anvika/anvika/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `anvika.json`:
...
- Use `anvika models list` and `anvika models set minimax/MiniMax-M2.1` to switch.
...
- Running `anvika configure` and selecting **MiniMax M2.1**, or
