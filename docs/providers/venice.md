---
summary: "Use Venice AI privacy-focused models in Anvika"
read_when:
  - You want privacy-focused inference in Anvika
...
```bash
anvika onboard --auth-choice venice-api-key
```
...
```bash
anvika agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, Anvika shows all available Venice models. Pick based on your needs:
...
```bash
anvika models set venice/claude-opus-45
anvika models set venice/llama-3.3-70b
```

List all available models:

```bash
anvika models list | grep venice
```

## Configure via `anvika configure`

1. Run `anvika configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Anvika automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
anvika agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
anvika agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
anvika agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
anvika agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
anvika agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `anvika models list` to see currently available models. Some models may be temporarily offline.
