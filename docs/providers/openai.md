---
summary: "Use OpenAI via API keys or Codex subscription in Anvika"
read_when:
  - You want to use OpenAI models in Anvika
...
```bash
anvika onboard --auth-choice openai-api-key
# or non-interactive
anvika onboard --openai-api-key "$OPENAI_API_KEY"
```
...
anvika onboard --auth-choice openai-codex

# Or run OAuth directly
anvika models auth login --provider openai-codex
```
...
Anvika uses `pi-ai` for model streaming. For `openai-codex/*` models you can set
...
`baseUrl` on `api.openai.com`), Anvika now auto-enables OpenAI server-side
