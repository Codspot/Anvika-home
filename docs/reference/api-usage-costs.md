...
Anvika features that can generate provider usage or paid API calls.
...
- `anvika status --usage` and `anvika channels list` show provider **usage windows**
...
Anvika can pick up credentials from:
...
`anvika models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
