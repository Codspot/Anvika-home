...
Anvika integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Anvika owns lifecycle when `channels.signal.autoStart=true`.
...
- Anvika spawns `imsg rpc` as a child process (legacy iMessage integration).
