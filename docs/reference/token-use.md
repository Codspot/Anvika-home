---
summary: "How Anvika builds prompt context and reports token usage + costs"
read_when:
...
Anvika tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Anvika assembles its own system prompt on every run. It includes:
...
For images, Anvika downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Anvika session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `anvika status --usage` and `anvika channels list` show
...
`cacheWrite`. If pricing is missing, Anvika shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Anvika can
...
Anthropic's 1M context window is currently beta-gated. Anvika can inject the
...
Anvika skips the `context-1m-*` beta header because Anthropic currently
