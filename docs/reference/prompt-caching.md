...
- With Anthropic API-key auth profiles, Anvika seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Anvika injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Anvika exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.anvika/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$ANVIKA_STATE_DIR/logs/cache-trace.jsonl`
...
- `ANVIKA_CACHE_TRACE=1` enables cache tracing.
- `ANVIKA_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `ANVIKA_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `ANVIKA_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `ANVIKA_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
