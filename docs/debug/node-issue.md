...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Anvika
...
Running Anvika via Node with `tsx` fails at startup with:

```
[anvika] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `anvika status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch anvika.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node anvika.mjs status` works on Node 25.
