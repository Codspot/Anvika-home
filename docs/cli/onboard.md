---
summary: "CLI reference for `anvika onboard` (interactive onboarding wizard)"
read_when:
...
```bash
anvika onboard
anvika onboard --flow quickstart
anvika onboard --flow manual
anvika onboard --mode remote --remote-url ws://gateway-host:18789
```
...
- Fastest first chat: `anvika dashboard` (Control UI, no channel setup).
