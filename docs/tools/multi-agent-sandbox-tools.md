...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `anvika sandbox explain`.
...
Legacy `agent.*` configs are migrated by `anvika doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=anvika-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${ANVIKA_STATE_DIR:-$HOME/.anvika}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
