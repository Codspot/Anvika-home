---
summary: "Agent tool surface for Anvika (browser, canvas, nodes, message, cron) replacing legacy `anvika-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `anvika-*` skills
title: "Tools"
---

# Tools (Anvika)

Anvika exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `anvika-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `anvika.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Anvika logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:anvika`: all built-in Anvika tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`anvika node run`).
...
Anvika tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `anvika configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Anvika-managed browser.
...
- Quick smoke: `anvika nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `anvika gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, Anvika uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Anvika clamps `tools.sessions.visibility` to `tree`.
