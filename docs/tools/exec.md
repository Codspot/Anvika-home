...
- `gateway`/`node` approvals are controlled by `~/.anvika/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Anvika prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`anvika security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `anvika doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
