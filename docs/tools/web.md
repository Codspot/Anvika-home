...
Anvika ships two lightweight web tools:
...
If no `provider` is explicitly set, Anvika auto-detects which provider to use based on available API keys, checking in this order:
...
3. Run `anvika configure --section web` to store the key in config (recommended), or set `BRAVE_API_KEY` in your environment.
...
**Recommended:** run `anvika configure --section web`. It stores the key in
`~/.anvika/anvika.json` under `tools.web.search.apiKey`.

**Environment alternative:** set `BRAVE_API_KEY` in the Gateway process
environment. For a gateway install, put it in `~/.anvika/.env` (or your
service environment). See [Env vars](/help/faq#how-does-anvika-load-environment-variables).
...
environment. For a gateway install, put it in `~/.anvika/.env`.

If no base URL is set, Anvika chooses a default based on the API key source:
...
For a gateway install, put it in `~/.anvika/.env`.
