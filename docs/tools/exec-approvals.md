...
- **gateway host** → `anvika` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.anvika/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.anvika/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `anvika approvals allowlist ...`).
- `anvika security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `anvika doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.anvika/exec-approvals.json` directly.

CLI: `anvika approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
