...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let Anvika run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask Anvika in plain language to "run this in Codex" or "start Claude Code in a thread", Anvika should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want Anvika-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Anvika native sub-agent runtime  |
...
- Anvika binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, Anvika returns a clear unsupported/unavailable message.
...
- If `agentId` is omitted, Anvika uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, Anvika may default to persistent behavior per runtime path
...
If no target resolves, Anvika returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, Anvika returns a clear unsupported-control error.
...
When Anvika uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Anvika `agentId` path).
...
```bash
anvika plugins install @anvika/acpx
anvika config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
anvika plugins install ./extensions/acpx
```
...
`@anvika/acpx` now enforces a strict plugin-local pinning model:
...
- Anvika startup stays non-blocking while acpx ensure runs.
