---
summary: "Chrome extension: let Anvika drive your existing Chrome tab"
read_when:
...
The Anvika Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate anvika-managed Chrome profile.
...
Anvika then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Anvika release (npm package) as static files. There is no separate “build” step.

After upgrading Anvika:

- Re-run `anvika browser extension install` to refresh the installed files under your Anvika state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Anvika ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `ANVIKA_GATEWAY_TOKEN`)
...
- Open the tab you want Anvika to control.
...
- `ON`: attached; Anvika can drive that tab.
...
Debugging: `anvika sandbox explain`
...
`anvika browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `anvika browser extension install` first to copy the extension to a stable location under your Anvika state directory.
...
- **This is not isolated** like the dedicated anvika-managed profile.
