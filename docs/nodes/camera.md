...
Anvika supports **camera capture** for agent workflows:
...
```bash
anvika nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
anvika nodes camera snap --node <id> --facing front
anvika nodes camera clip --node <id> --duration 3000
anvika nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`anvika.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `anvika` CLI to invoke camera commands on the macOS node.

Examples:

```bash
anvika nodes camera list --node <id>            # list camera ids
anvika nodes camera snap --node <id>            # prints MEDIA:<path>
anvika nodes camera snap --node <id> --max-width 1280
anvika nodes camera snap --node <id> --delay-ms 2000
anvika nodes camera snap --node <id> --device-id <id>
anvika nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
anvika nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
anvika nodes camera clip --node <id> --device-id <id>
anvika nodes camera clip --node <id> --no-audio
```

Notes:

- `anvika nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
anvika nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
