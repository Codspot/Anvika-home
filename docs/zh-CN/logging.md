...
你可以在 `~/.anvika/anvika.json` 中覆盖此设置：
...
所有日志配置都在 `~/.anvika/anvika.json` 的 `logging` 下。
...
- Anvika 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `anvika plugins enable diagnostics-otel` 启用插件。
...
- `anvika.tokens`（计数器，属性：`anvika.token`、`anvika.channel`、`anvika.provider`、`anvika.model`）
- `anvika.cost.usd`（计数器，属性：`anvika.channel`、`anvika.provider`、`anvika.model`）
- `anvika.run.duration_ms`（直方图，属性：`anvika.channel`、`anvika.provider`、`anvika.model`）
- `anvika.context.tokens`（直方图，属性：`anvika.context`、`anvika.channel`、`anvika.provider`、`anvika.model`）

消息流：

- `anvika.webhook.received`（计数器，属性：`anvika.channel`、`anvika.webhook`）
- `anvika.webhook.error`（计数器，属性：`anvika.channel`、`anvika.webhook`）
- `anvika.webhook.duration_ms`（直方图，属性：`anvika.channel`、`anvika.webhook`）
- `anvika.message.queued`（计数器，属性：`anvika.channel`、`anvika.source`）
- `anvika.message.processed`（计数器，属性：`anvika.channel`、`anvika.outcome`）
- `anvika.message.duration_ms`（直方图，属性：`anvika.channel`、`anvika.outcome`）

队列 + 会话：

- `anvika.queue.lane.enqueue`（计数器，属性：`anvika.lane`）
- `anvika.queue.lane.dequeue`（计数器，属性：`anvika.lane`）
- `anvika.queue.depth`（直方图，属性：`anvika.lane` 或 `anvika.channel=heartbeat`）
- `anvika.queue.wait_ms`（直方图，属性：`anvika.lane`）
- `anvika.session.state`（计数器，属性：`anvika.state`、`anvika.reason`）
- `anvika.session.stuck`（计数器，属性：`anvika.state`）
- `anvika.session.stuck_age_ms`（直方图，属性：`anvika.state`）
- `anvika.run.attempt`（计数器，属性：`anvika.attempt`）
...
- `anvika.webhook.error`
  - `anvika.channel`、`anvika.webhook`、`anvika.chatId`、`anvika.error`
- `anvika.message.processed`
  - `anvika.channel`、`anvika.outcome`、`anvika.chatId`、`anvika.messageId`、`anvika.sessionKey`、`anvika.sessionId`、`anvika.reason`
- `anvika.session.stuck`
  - `anvika.state`、`anvika.ageMs`、`anvika.queueDepth`、`anvika.sessionKey`、`anvika.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `anvika doctor`。
