...
- `/usage cost` in chats: local cost summary aggregated from Anvika session logs.
- CLI: `anvika status --usage` prints a full per-provider breakdown.
- CLI: `anvika channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
