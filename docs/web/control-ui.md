...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/anvika`)
...
If the page fails to load, start the Gateway first: `anvika gateway`.
...
you revoke it with `anvika devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.anvika/anvika.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop anvika`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Anvika
...
Anvika **blocks** Control UI connections without device identity.
