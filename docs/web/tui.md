...
```bash
anvika tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `anvika tui --deliver`
...
- Check the Gateway logs: `anvika logs --follow`.
- Confirm the agent can run: `anvika status` and `anvika models status`.
...
- No agents in picker: check `anvika agents list` and your routing config.
