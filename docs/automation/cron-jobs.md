...
- Jobs persist under `~/.anvika/cron/` so restarts don’t lose schedules.
...
anvika cron list
anvika cron run <job-id>
anvika cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.anvika/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `anvika cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Anvika applies a
...
If `delivery` is omitted for isolated jobs, Anvika defaults to `announce`.
...
- Job store: `~/.anvika/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.anvika/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
- For removed run sessions no longer referenced by the session store, Anvika archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `anvika cron runs` and adjust retention before logs become large
...
anvika cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
anvika cron edit <jobId> --agent ops
anvika cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
anvika cron run <jobId>
anvika cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
anvika cron edit <jobId> \
...
```bash
anvika system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`anvika system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `ANVIKA_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Anvika applies exponential retry backoff for recurring jobs after consecutive errors:
