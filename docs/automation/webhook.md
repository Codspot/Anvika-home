...
read_when:
  ...
  - Wiring external systems into Anvika
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Anvika config directory (typically `~/.anvika/hooks/transforms`).
...
- `anvika webhooks gmail setup` writes `hooks.gmail` config for `anvika webhooks gmail run`.
