---
summary: "Anvika on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Anvika on a Raspberry Pi
  - Running Anvika on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Anvika on Raspberry Pi

## Goal

Run a persistent, always-on Anvika Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Anvika features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled anvika

# Enable if not
sudo systemctl enable anvika

# Start on boot
sudo systemctl start anvika
```
...
```bash
# Check logs
journalctl -u anvika --no-pager -n 100

# Common fix: rebuild
cd ~/anvika  # if using hackable install
npm run build
sudo systemctl restart anvika
```
