---
summary: "Anvika on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Anvika on DigitalOcean
  - Looking for cheap VPS hosting for Anvika
title: "DigitalOcean"
---

# Anvika on DigitalOcean

## Goal

Run a persistent Anvika Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status anvika-gateway.service

# View logs
journalctl --user -u anvika-gateway.service -f
```
...
```bash
anvika config set gateway.bind tailnet
anvika gateway restart
```
...
```bash
anvika channels login whatsapp
# Scan QR code
```
...
- `~/.anvika/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u anvika --no-pager -n 50
```
