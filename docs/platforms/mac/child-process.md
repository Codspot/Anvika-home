...
service via the external `anvika` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.anvika.gateway`
  (or `ai.anvika.<profile>` when using `--profile`/`ANVIKA_PROFILE`; legacy `com.anvika.*` is supported).
