...
- Same path: run the app from a fixed location (for Anvika, `dist/Anvika.app`).
...
```bash
sudo tccutil reset Accessibility ai.anvika.mac
sudo tccutil reset ScreenCapture ai.anvika.mac
...
Workaround: move files into the Anvika workspace (`~/.anvika/workspace`) if you want to avoid per-folder grants.
