...
summary: "Architecture of Anvika's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Anvika
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how Anvika integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Anvika uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Anvika directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Anvika's local TUI mode)                                             |
...
├── pi-tools.ts                    # createAnvikaCodingTools()
...
├── anvika-tools.ts              # Anvika-specific tools
...
Image injection is prompt-local: Anvika loads image refs from the current prompt and
...
2. **Custom Replacements**: Anvika replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Anvika Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Anvika's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Anvika CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Anvika wraps this with `guardSessionManager()` for tool result safety.
...
Anvika maintains an auth profile store with multiple API keys per provider:
...
Anvika loads custom pi extensions for specialized behavior:
...
Anvika also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Anvika Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Anvika tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.anvika/agents/<agentId>/sessions/` (or `$ANVIKA_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `ANVIKA_LIVE_TEST=1`)
