...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Anvika to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
anvika plugins install @anvika/feishu
```

Local checkout (when running from a git repo):

```bash
anvika plugins install ./extensions/feishu
```
...
If you just installed Anvika, run the wizard:
...
2. Configuring app credentials in Anvika
...
1. You already ran `anvika channels add` for Feishu
2. The gateway is running (`anvika gateway status`)
...
```bash
anvika pairing approve feishu <CODE>
```
...
2. Run `anvika logs --follow` and look for `chat_id`
...
2. Run `anvika logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `anvika gateway status`  | Show gateway status           |
| `anvika gateway install` | Install/start gateway service |
| `anvika gateway stop`    | Stop gateway service          |
| `anvika gateway restart` | Restart gateway service       |
| `anvika logs --follow`   | Tail gateway logs             |
...
4. Check logs: `anvika logs --follow`
...
5. Ensure the gateway is running: `anvika gateway status`
6. Check logs: `anvika logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.anvika/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.anvika/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
