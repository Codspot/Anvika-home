...
Tlon is a decentralized messenger built on Urbit. Anvika connects to your Urbit ship and can
...
```bash
anvika plugins install @anvika/tlon
```

Local checkout (when running from a git repo):

```bash
anvika plugins install ./extensions/tlon
```
...
By default, Anvika blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `anvika message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Anvika replies in-thread.
