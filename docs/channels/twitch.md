...
read_when:
  - Setting up Twitch chat integration for Anvika
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Anvika connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
anvika plugins install @anvika/twitch
```

Local checkout (when running from a git repo):

```bash
anvika plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `ANVIKA_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "anvika", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ANVIKA_TWITCH_ACCESS_TOKEN env var)
...
```bash
ANVIKA_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
