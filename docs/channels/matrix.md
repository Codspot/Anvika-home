...
Matrix is an open, decentralized messaging protocol. Anvika connects as a Matrix **user**
...
```bash
anvika plugins install @anvika/matrix
```

Local checkout (when running from a git repo):

```bash
anvika plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `anvika plugins install @anvika/matrix`
   - From a local checkout: `anvika plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Anvika calls the same
     login endpoint, stores the access token in `~/.anvika/credentials/matrix/credentials.json`,
...
- On first connection, Anvika requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Anvika logs a warning.
...
`~/.anvika/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `anvika pairing list matrix`
  - `anvika pairing approve matrix <CODE>`
...
- On startup, Anvika resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
