...
```bash
anvika plugins install @anvika/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
anvika plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Anvika" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `anvika pairing list nextcloud-talk`
  - `anvika pairing approve nextcloud-talk <CODE>`
