...
read_when:
  - You want Anvika to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Anvika to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`anvika onboard`) and `anvika channels add` list optional channel plugins.
...
```bash
anvika plugins install @anvika/nostr
```

Use a local checkout (dev workflows):

```bash
anvika plugins install --link <path-to-anvika>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "anvika@example.com",
        "lud16": "anvika@example.com"
      }
    }
  }
}
