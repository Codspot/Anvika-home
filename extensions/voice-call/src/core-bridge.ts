...
function resolveAnvikaRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.ANVIKA_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set ANVIKA_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveAnvikaRoot(), "dist", "extensionAPI.js");
