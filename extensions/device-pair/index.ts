...
function resolveGatewayPort(cfg: AnvikaPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.ANVIKA_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: AnvikaPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: AnvikaPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Anvika iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Anvika iOS app:",
            "",
            "```",
            qrAscii,
            "```",
