...
import { createDedupeCache, createPersistentDedupe } from "anvika/plugin-sdk";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.ANVIKA_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["anvika-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".anvika");
}
