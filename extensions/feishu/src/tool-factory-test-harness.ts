import type { AnyAgentTool, AnvikaPluginApi } from "anvika/plugin-sdk";
...
export function createToolFactoryHarness(cfg: AnvikaPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<AnvikaPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
