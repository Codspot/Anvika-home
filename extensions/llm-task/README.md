...
`anvika.invoke --each`) without adding new Anvika code per workflow.
...
This extension depends on Anvika internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Anvika extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.anvika/extensions` as a standalone plugin directory.
