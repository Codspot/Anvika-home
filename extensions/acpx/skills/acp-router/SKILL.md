---
name: acp-router
description: Route plain-language requests for Pi, Claude Code, Codex, OpenCode, Gemini CLI, or ACP harness work into either Anvika ACP runtime sessions or direct acpx-driven sessions ("telephone game" flow).
...
Trigger this skill when the user asks Anvika to:
...
1. Anvika ACP runtime path (default): use `sessions_spawn` / ACP runtime tools.
...
- the task is "just relay prompts to harness" and no Anvika ACP lifecycle features are needed
...
For this repo, direct `acpx` calls must follow the same pinned policy as the `@anvika/acpx` extension.
