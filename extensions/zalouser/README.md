...
Anvika extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
anvika channels login --channel zalouser
...
```bash
anvika message send --channel zalouser --target <threadId> --message "Hello from Anvika!"
```
...
```bash
anvika channels login --channel zalouser              # Login via QR
anvika channels login --channel zalouser --account work
anvika channels status --probe
anvika channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
anvika directory self --channel zalouser
anvika directory peers list --channel zalouser --query "name"
anvika directory groups list --channel zalouser --query "work"
anvika directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
anvika message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
anvika message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `anvika channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
anvika channels login --channel zalouser --account work
anvika message send --channel zalouser --account work --target <id> --message "Hello"
