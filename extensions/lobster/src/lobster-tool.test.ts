...
import type { AnvikaPluginApi, AnvikaPluginToolContext } from "../../../src/plugins/types.js";
import {
...
function fakeApi(overrides: Partial<AnvikaPluginApi> = {}): AnvikaPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<AnvikaPluginToolContext> = {}): AnvikaPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: AnvikaPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
