...
import type { AnvikaPluginApi, AnvikaPluginService } from "anvika/plugin-sdk";
...
function normalizeDenyList(cfg: AnvikaPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: AnvikaPluginApi["config"]): string[] {
...
export default function register(api: AnvikaPluginApi) {
  ...
  const timerService: AnvikaPluginService = {
