...
metadata: { "anvika": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Anvika `sessions_spawn`
- **File I/O** == Anvika `read`/`write`
- **Remote fetch** == Anvika `web_fetch` (or `exec` with curl when POST is required)
