...
vi.mock("anvika/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("anvika/plugin-sdk")>("anvika/plugin-sdk");
  return {
    ...actual,
...
import type { AnvikaPluginServiceContext } from "anvika/plugin-sdk";
import { emitDiagnosticEvent } from "anvika/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/anvika-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): AnvikaPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("anvika.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("anvika.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("anvika.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("anvika.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("anvika.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("anvika.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("anvika.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("anvika.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("anvika.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("anvika.webhook.processed");
    expect(spanNames).toContain("anvika.message.processed");
    expect(spanNames).toContain("anvika.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["anvika.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("anvika.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "anvika.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["anvika.reason"]).toBe("string");
    expect(String(attrs?.["anvika.reason"])).not.toContain(
