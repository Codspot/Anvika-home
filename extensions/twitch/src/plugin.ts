/**
 * Twitch channel plugin for Anvika.
 *
...
import { buildChannelConfigSchema } from "anvika/plugin-sdk";
 ...
 * for Anvika. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: AnvikaConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: AnvikaConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: AnvikaConfig): boolean => {
