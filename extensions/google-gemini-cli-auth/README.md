# Google Gemini CLI Auth (Anvika plugin)
...
```bash
anvika plugins enable google-gemini-cli-auth
```
...
```bash
anvika models auth login --provider google-gemini-cli --set-default
```
...
- `ANVIKA_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `ANVIKA_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
