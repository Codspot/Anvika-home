...
describe("loginGeminiCliOAuth", () => {
  ...
  const ENV_KEYS = [
    "ANVIKA_GEMINI_OAUTH_CLIENT_ID",
    "ANVIKA_GEMINI_OAUTH_CLIENT_SECRET",
  ...
  beforeEach(() => {
    ...
    process.env.ANVIKA_GEMINI_OAUTH_CLIENT_ID = "test-client-id.apps.googleusercontent.com";
    process.env.ANVIKA_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret";
  ...
  it("falls back across loadCodeAssist endpoints with aligned headers and metadata", async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === USERINFO_URL) {
        return responseJson({ email: "lobster@anvika.ai" });
      }
      if (url === LOAD_PROD) {
