---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Anvika deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Anvika cron scheduling for periodic checks, or version status checks on a machine running Anvika (laptop, workstation, Pi, VPS).
---

# Anvika Host Hardening

## Overview

Assess and harden the host running Anvika, then align it to a user-defined risk tolerance without breaking access. Use Anvika security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Anvika changes the host firewall, SSH, or OS updates; it does not.
...
5. Anvika gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Anvika can access sensitive data.
...
### 2) Run Anvika security audits (read-only)

As part of the default read-only checks, run `anvika security audit --deep`. Only offer alternatives if the user requests them:

1. `anvika security audit` (faster, non-probing)
2. `anvika security audit --json` (structured output)

Offer to apply Anvika safe defaults (numbered):

1. `anvika security audit --fix`

Be explicit that `--fix` only tightens Anvika defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Anvika version/update status (read-only)

As part of the default read-only checks, run `anvika update status`.
...
- Credential hygiene notes (location of Anvika creds, prefer disk encryption)
...
After Anvika install or first hardening pass, run at least one baseline audit and version check:

- `anvika security audit`
- `anvika security audit --deep`
- `anvika update status`

Ongoing monitoring is recommended. Use the Anvika cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `anvika cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `anvika update status`
...
Before creating, `anvika cron list` and match on exact `name`. If found, `anvika cron edit <id> ...`.
If not found, `anvika cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `anvika update status` (preferred for source checkouts and channels)
2. `npm view anvika version` (published npm version)

## Anvika command accuracy

Use only supported commands and flags:

- `anvika security audit [--deep] [--fix] [--json]`
- `anvika status` / `anvika status --deep`
- `anvika health --json`
- `anvika update status`
- `anvika cron add|list|runs|run`

Do not invent CLI flags or imply Anvika enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Anvika compaction:
