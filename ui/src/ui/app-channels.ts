...
export async function handleWhatsAppStart(host: AnvikaApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: AnvikaApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: AnvikaApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: AnvikaApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: AnvikaApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: AnvikaApp): string {
...
function resolveGatewayHttpAuthHeader(host: AnvikaApp): string | null {
...
function buildGatewayHttpHeaders(host: AnvikaApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: AnvikaApp) {
...
export function handleNostrProfileToggleAdvanced(host: AnvikaApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: AnvikaApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: AnvikaApp) {
  const state = host.nostrProfileFormState;
