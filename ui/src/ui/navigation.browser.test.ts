...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/anvika/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/anvika");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/anvika/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ANVIKA_CONTROL_UI_BASE_PATH__ = "/anvika";
    const app = mountApp("/anvika/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/anvika");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/anvika/sessions");
  });
