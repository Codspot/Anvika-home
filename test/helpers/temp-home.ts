...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    anvikaHome: process.env.ANVIKA_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("ANVIKA_HOME", snapshot.anvikaHome);
  restoreKey("ANVIKA_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked ANVIKA_HOME.
  delete process.env.ANVIKA_HOME;
  process.env.ANVIKA_STATE_DIR = path.join(base, ".anvika");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "anvika-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".anvika", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
