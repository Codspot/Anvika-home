...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `anvika-e2e-${name}-`));
  const configDir = path.join(homeDir, ".anvika");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          ANVIKA_SKIP_PROVIDERS: "1",
          ANVIKA_SKIP_GMAIL_WATCHER: "1",
          ANVIKA_SKIP_CRON: "1",
          ANVIKA_SKIP_BROWSER_CONTROL_SERVER: "1",
          ANVIKA_SKIP_CANVAS_HOST: "1",
          ANVIKA_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
