...
final class MotionService: MotionServicing {
    func activities(params: AnvikaMotionActivityParams) async throws -> AnvikaMotionActivityPayload {
        ...
        let mapped: [AnvikaMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return AnvikaMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: AnvikaPedometerParams) async throws -> AnvikaPedometerPayload {
        ...
        let payload: AnvikaPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = AnvikaPedometerPayload(
