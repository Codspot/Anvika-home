...
final class RemindersService: RemindersServicing {
    func list(params: AnvikaRemindersListParams) async throws -> AnvikaRemindersListPayload {
        ...
        let payload: [AnvikaReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return AnvikaRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: AnvikaRemindersAddParams) async throws -> AnvikaRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = AnvikaReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return AnvikaRemindersAddPayload(reminder: payload)
    }
