...
final class ContactsService: ContactsServicing {
    ...
    func search(params: AnvikaContactsSearchParams) async throws -> AnvikaContactsSearchPayload {
        ...
        return AnvikaContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: AnvikaContactsAddParams) async throws -> AnvikaContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return AnvikaContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return AnvikaContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> AnvikaContactPayload {
        AnvikaContactPayload(
