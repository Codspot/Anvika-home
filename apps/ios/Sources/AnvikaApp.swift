...
@MainActor
final class AnvikaAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.anvika.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.anvika.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.anvika.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "anvika.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "anvika.watch.promptId"
    static let sessionKeyKey = "anvika.watch.sessionKey"
    static let actionPrimaryIDKey = "anvika.watch.action.primary.id"
    static let actionPrimaryLabelKey = "anvika.watch.action.primary.label"
    static let actionSecondaryIDKey = "anvika.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "anvika.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "anvika.watch.action.primary"
    static let actionSecondaryIdentifier = "anvika.watch.action.secondary"
    static let actionIdentifierPrefix = "anvika.watch.action."
    static let actionIDKeyPrefix = "anvika.watch.action.id."
    static let actionLabelKeyPrefix = "anvika.watch.action.label."
    static let categoryPrefix = "anvika.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: AnvikaWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> AnvikaWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AnvikaWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Anvika" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [AnvikaWatchAction]) -> [UNNotificationAction] {
...
@main
struct AnvikaApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(AnvikaAppDelegate.self) private var appDelegate
...
extension AnvikaApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Anvika: installing uncaught exception handler")
