...
struct IOSGatewayChatTransport: AnvikaChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.anvika", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> AnvikaChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(AnvikaChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> AnvikaChatHistoryPayload {
        ...
        return try JSONDecoder().decode(AnvikaChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [AnvikaChatAttachmentPayload]) async throws -> AnvikaChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [AnvikaChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(AnvikaChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(AnvikaGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AnvikaGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AnvikaChatEventPayload.self)
                        {
