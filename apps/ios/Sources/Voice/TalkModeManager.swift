...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__ANVIKA_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<AnvikaTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.anvika", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = AnvikaTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> AnvikaTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return AnvikaTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> AnvikaTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> AnvikaTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return AnvikaTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> AnvikaTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = AnvikaTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: AnvikaTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: AnvikaAgentEventPayload.self
            ) else {
                continue
            }
