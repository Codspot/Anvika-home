...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> AnvikaDeviceStatusPayload {
        ...
        return AnvikaDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> AnvikaDeviceInfoPayload {
        ...
        return AnvikaDeviceInfoPayload(
    ...
    private func batteryStatus() -> AnvikaBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: AnvikaBatteryState = switch device.batteryState {
        ...
        return AnvikaBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> AnvikaThermalStatusPayload {
        let state: AnvikaThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return AnvikaThermalStatusPayload(state: state)
    }

    private func storageStatus() -> AnvikaStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return AnvikaStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
