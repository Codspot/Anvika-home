...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: AnvikaWatchNotifyParams) -> AnvikaWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [AnvikaWatchAction]?,
        kind: String?,
        promptId: String?) -> [AnvikaWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> AnvikaWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AnvikaWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                AnvikaWatchAction(id: "approve", label: "Approve"),
                AnvikaWatchAction(id: "decline", label: "Decline", style: "destructive"),
                AnvikaWatchAction(id: "open_phone", label: "Open iPhone"),
                AnvikaWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            AnvikaWatchAction(id: "done", label: "Done"),
            AnvikaWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            AnvikaWatchAction(id: "open_phone", label: "Open iPhone"),
            AnvikaWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: AnvikaWatchRisk?,
        priority: AnvikaNotificationPriority?) -> AnvikaWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: AnvikaNotificationPriority?,
        risk: AnvikaWatchRisk?) -> AnvikaNotificationPriority?
    {
