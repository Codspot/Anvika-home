...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.anvika.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Anvika", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Anvika gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Anvika.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "AnvikaShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Anvika is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Anvika Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("anvika-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
