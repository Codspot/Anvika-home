...
struct AnvikaMacCLI {
    static func main() async {
        ...
        default:
            fputs("anvika-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      anvika-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      anvika-mac discover [--timeout <ms>] [--json] [--include-local]
      anvika-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      anvika-mac connect
      anvika-mac connect --url ws://127.0.0.1:18789 --json
      anvika-mac discover --timeout 3000 --json
      anvika-mac wizard --mode local
    """)
}
