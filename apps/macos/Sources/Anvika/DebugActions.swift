...
enum DebugActions {
    private static let verboseDefaultsKey = "anvika.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "anvika.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = AnvikaPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Anvika", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = AnvikaPaths.configURL
        guard
