...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: AnvikaProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.anvika", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AnvikaKit.AnyCodable]()) {
                $0[$1.key] = AnvikaKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AnvikaProtocol.AnyCodable?) -> [String: AnvikaProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AnvikaProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AnvikaKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AnvikaProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("anvika.control.heartbeat")
    static let controlAgentEvent = Notification.Name("anvika.control.agent")
}
