...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.anvika", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: AnvikaNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [AnvikaCapability.canvas.rawValue, AnvikaCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(AnvikaCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if AnvikaLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(AnvikaCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            AnvikaSystemCommand.which.rawValue,
            AnvikaSystemCommand.run.rawValue,
            AnvikaSystemCommand.execApprovalsGet.rawValue,
            AnvikaSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(AnvikaCapability.camera.rawValue) {
        ...
        if capsSet.contains(AnvikaCapability.location.rawValue) {
