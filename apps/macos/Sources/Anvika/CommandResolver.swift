...
enum CommandResolver {
    private static let projectRootDefaultsKey = "anvika.gatewayProjectRootPath"
    private static let helperName = "anvika"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let anvikaEntry = root.appendingPathComponent("anvika.mjs").path
        if FileManager().isReadableFile(atPath: anvikaEntry) { return anvikaEntry }
        let binEntry = root.appendingPathComponent("bin/anvika.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/anvika")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let anvikaPaths = self.anvikaManagedPaths(home: home)
        if !anvikaPaths.isEmpty {
            extras.insert(contentsOf: anvikaPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + anvikaPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func anvikaManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".anvika"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func anvikaExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectAnvikaExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("anvika.mjs").path,
            root.appendingPathComponent("bin/anvika.js").path,
        ]
    ...
    static func hasAnyAnvikaInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.anvikaExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let anvikaPath = self.projectAnvikaExecutable(projectRoot: root) {
            return [anvikaPath, subcommand] + extraArgs
        }
        if let anvikaPath = self.anvikaExecutable(searchPaths: searchPaths) {
            return [anvikaPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "anvika", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            anvika entrypoint missing (looked for dist/index.js or anvika.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real anvika CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/anvika"
        ...
        \(cliSection)
        \(projectSection)
        if command -v anvika >/dev/null 2>&1; then
          CLI="$(command -v anvika)"
          anvika \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/anvika.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/anvika.mjs"
            node "$PRJ/anvika.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/anvika.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/anvika.js"
            node "$PRJ/bin/anvika.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent anvika"
          pnpm --silent anvika \(quotedArgs);
        else
          echo "anvika CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? AnvikaConfigFile.loadDict()
