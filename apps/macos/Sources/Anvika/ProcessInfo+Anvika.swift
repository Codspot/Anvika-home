...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.anvika.mac`) even if the shipped
    ...
    {
        if environment["ANVIKA_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "anvika.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "anvika.nixMode") { return true }
