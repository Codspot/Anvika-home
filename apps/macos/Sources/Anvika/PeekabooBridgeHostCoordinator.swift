...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.anvika", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: AnvikaPeekabooBridgeServices?
    private static var anvikaSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Anvika", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = AnvikaPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.anvikaSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.anvikaSocketPath, privacy: .public)")
    }
...
@MainActor
private final class AnvikaPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.anvika.peekaboo")
