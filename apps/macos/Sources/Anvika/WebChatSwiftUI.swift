...
private let webChatSwiftLogger = Logger(subsystem: "ai.anvika", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: AnvikaChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> AnvikaChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(AnvikaChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> AnvikaChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                AnvikaGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<AnvikaChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any AnvikaChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = AnvikaChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: AnvikaChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<AnvikaChatView>) -> NSViewController
    {
