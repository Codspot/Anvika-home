...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ANVIKA_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/anvika")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("anvika-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("anvika-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/anvika/ (rolling or stdout), or nil if none exist.
