...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.anvika/anvika.json. Prefer ANVIKA_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.anvika/anvika.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.anvika/anvika.json. Restart the gateway to apply."
        }
