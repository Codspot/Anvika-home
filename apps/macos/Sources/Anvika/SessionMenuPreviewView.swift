...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.anvika", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> AnvikaSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: AnvikaSessionPreviewEntry,
    ...
    private static func cache(payload: AnvikaSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: AnvikaChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [AnvikaKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [AnvikaKit.AnyCodable]) -> [AnvikaChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(AnvikaChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: AnvikaChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: AnvikaChatMessage) -> Bool {
    ...
    private static func toolNames(for message: AnvikaChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: AnvikaChatMessage) -> String? {
