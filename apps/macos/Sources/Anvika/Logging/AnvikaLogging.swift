...
enum AnvikaLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = AnvikaOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = AnvikaFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        AnvikaLogging.bootstrapIfNeeded()
        let label = AnvikaLogging.makeLabel(subsystem: subsystem, category: category)
...
struct AnvikaOSLogHandler: LogHandler {
...
struct AnvikaFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = AnvikaLogging.parseLabel(self.label)
