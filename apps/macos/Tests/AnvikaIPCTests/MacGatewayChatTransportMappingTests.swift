...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: AnvikaProtocol.AnyCodable(["ok": AnvikaProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: AnvikaProtocol.AnyCodable(["ok": AnvikaProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = AnvikaProtocol.AnyCodable([
            "runId": AnvikaProtocol.AnyCodable("run-1"),
            "sessionKey": AnvikaProtocol.AnyCodable("main"),
            "state": AnvikaProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: AnvikaProtocol.AnyCodable(["a": AnvikaProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
