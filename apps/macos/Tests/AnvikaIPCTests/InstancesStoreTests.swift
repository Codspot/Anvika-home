...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: AnvikaProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: AnvikaProtocol.AnyCodable] = [
            "presence": .init([AnvikaProtocol.AnyCodable(entry)]),
        ]
        let payload = AnvikaProtocol.AnyCodable(payloadMap)
