...
@MainActor
struct AnvikaChatComposer: View {
    @Bindable var viewModel: AnvikaChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(AnvikaChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(AnvikaChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \AnvikaPendingAttachment.id)
                { (att: AnvikaPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            AnvikaPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AnvikaChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(AnvikaChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(AnvikaChatTheme.subtleCard)
        .clipShape(Capsule())
    }
