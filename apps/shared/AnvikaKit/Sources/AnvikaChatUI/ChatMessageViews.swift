...
struct ChatMessageBubble: View {
    let message: AnvikaChatMessage
    let style: AnvikaChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? AnvikaChatTheme.userText : AnvikaChatTheme.assistantText
    ...
    private var inlineAttachments: [AnvikaChatMessageContent] {
    ...
    private var toolCalls: [AnvikaChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [AnvikaChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? AnvikaChatTheme.userBubble
        }
        if self.style == .onboarding {
            return AnvikaChatTheme.onboardingAssistantBubble
        }
        return AnvikaChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return AnvikaChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: AnvikaChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? AnvikaChatTheme.userText : AnvikaChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: AnvikaChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AnvikaChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(AnvikaChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [AnvikaChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: AnvikaChatTheme.assistantText)
            }
        }
    }
}
