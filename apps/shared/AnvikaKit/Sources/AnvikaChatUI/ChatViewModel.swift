...
private let chatUILogger = Logger(subsystem: "ai.anvika", category: "AnvikaChatUI")

@MainActor
@Observable
public final class AnvikaChatViewModel {
    public private(set) var messages: [AnvikaChatMessage] = []
    ...
    public var attachments: [AnvikaPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [AnvikaChatPendingToolCall] = []
    public private(set) var sessions: [AnvikaChatSessionEntry] = []
    private let transport: any AnvikaChatTransport
    ...
    private var pendingToolCallsById: [String: AnvikaChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any AnvikaChatTransport) {
    ...
    public var sessionChoices: [AnvikaChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [AnvikaChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: AnvikaPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [AnvikaChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: AnvikaChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: AnvikaChatMessage) -> AnvikaChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> AnvikaChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return AnvikaChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: AnvikaChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [AnvikaChatMessage],
        incoming: [AnvikaChatMessage]) -> [AnvikaChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [AnvikaChatMessage]) -> [AnvikaChatMessage] {
        var result: [AnvikaChatMessage] = []
    ...
    private static func dedupeKey(for message: AnvikaChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [AnvikaChatMessageContent] = [
            AnvikaChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> AnvikaChatAttachmentPayload in
            AnvikaChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> AnvikaChatSessionEntry {
        AnvikaChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: AnvikaChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: AnvikaChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: AnvikaAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = AnvikaChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> AnvikaPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
