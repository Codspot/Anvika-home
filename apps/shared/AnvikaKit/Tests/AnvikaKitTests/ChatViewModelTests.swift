...
private final class TestChatTransport: @unchecked Sendable, AnvikaChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [AnvikaChatHistoryPayload]
    private let sessionsResponses: [AnvikaChatSessionsListResponse]

    private let stream: AsyncStream<AnvikaChatTransportEvent>
    private let continuation: AsyncStream<AnvikaChatTransportEvent>.Continuation

    init(
        historyResponses: [AnvikaChatHistoryPayload],
        sessionsResponses: [AnvikaChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<AnvikaChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> AnvikaChatHistoryPayload {
        ...
        return self.historyResponses.last ?? AnvikaChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [AnvikaChatAttachmentPayload]) async throws -> AnvikaChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return AnvikaChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> AnvikaChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? AnvikaChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: AnvikaChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = AnvikaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = AnvikaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { AnvikaChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = AnvikaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = AnvikaChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = AnvikaChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { AnvikaChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
