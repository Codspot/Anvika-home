...
import "@a2ui/lit/ui";
import { themeContext } from "@anvika/a2ui-theme-context";
...
class AnvikaA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--anvika-a2ui-inset-top, 0px)
        var(--anvika-a2ui-inset-right, 0px)
        var(--anvika-a2ui-inset-bottom, 0px)
        var(--anvika-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--anvika-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--anvika-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--anvika-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--anvika-a2ui-empty-top, var(--anvika-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.anvikaA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["anvika:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__anvikaLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.anvikaCanvasA2UIAction ??
      globalThis.anvikaCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.anvikaCanvasA2UIAction) {
...
if (!customElements.get("anvika-a2ui-host")) {
  customElements.define("anvika-a2ui-host", AnvikaA2UIHost);
}
