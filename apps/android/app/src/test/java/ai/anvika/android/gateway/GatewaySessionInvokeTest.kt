...
class GatewaySessionInvokeTest {
  ...
  fun refreshNodeCanvasCapability_sendsObjectParamsAndUpdatesScopedUrl() = runBlocking {
                ...
                object : WebSocketListener() {
                  ...
                  override fun onMessage(webSocket: WebSocket, text: String) {
                    ...
                    when (method) {
                      "connect" -> {
                        webSocket.send(
                          """{"type":"res","id":"$id","ok":true,"payload":{"canvasHostUrl":"http://127.0.0.1/__anvika__/cap/old-cap","snapshot":{"sessionDefaults":{"mainSessionKey":"main"}}}}""",
                        )
                      }
    ...
    try {
      ...
      assertEquals(
        "http://127.0.0.1:${server.port}/__anvika__/cap/new-cap",
