...
private const val NOTIFICATION_CHANNEL_BASE_ID = "anvika.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Anvika Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Anvika Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Anvika Active")
      }
