...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "Anvika Node", text = "Starting…")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title = if (connected) "Anvika Node · Connected" else "Anvika Node"
          val micSuffix =
            if (micEnabled) {
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "Anvika node connection status"
        setShowBadge(false)
      }
  ...
  companion object {
    ...
    private const val ACTION_STOP = "ai.anvika.android.action.STOP"
