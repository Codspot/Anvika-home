#!/bin/bash
# Setup Anvika Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'anvika-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Anvika message: Send warning via Anvika itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/anvika-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/anvika-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/anvika-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now anvika-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status anvika-auth-monitor.timer"
echo ""
echo "Quick commands:"
