...
function ensureChangelogEntry(changelogPath: string, version: string): boolean {
  ...
  const entry = `## ${version}\n\n### Changes\n- Version alignment with core Anvika release numbers.\n\n`;
...
function stripWorkspaceAnvikaDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.anvika !== "workspace:*") {
    return false;
  }
  delete devDeps.anvika;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceAnvikaDevDependency(pkg);
