...
IMAGE_NAME="anvika-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="anvika-net-e2e-$$"
GW_NAME="anvika-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "ANVIKA_GATEWAY_TOKEN=$TOKEN" \
  -e "ANVIKA_SKIP_CHANNELS=1" \
  -e "ANVIKA_SKIP_GMAIL_WATCHER=1" \
  -e "ANVIKA_SKIP_CRON=1" \
  -e "ANVIKA_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
