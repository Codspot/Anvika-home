...
function runTests() {
  const isolatedLock =
    process.env.ANVIKA_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `anvika-gateway.lock.test.${Date.now()}`);
  const result = spawnSync("pnpm", ["vitest", "run"], {
    stdio: "inherit",
    env: {
      ...process.env,
      ANVIKA_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
...
function main() {
  const port = Number.parseInt(process.env.ANVIKA_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
