...
function resolveStateDir(): string {
  const override = process.env.ANVIKA_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.ANVIKA_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".anvika");
}
...
function usage(): string {
  return (
    ...
    "1) Anvika spawned an ACP thread binding\n" +
    ...
    "  ANVIKA_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  ANVIKA_DISCORD_SMOKE_DRIVER\n" +
    "  ANVIKA_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  ANVIKA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  ANVIKA_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  ANVIKA_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  ANVIKA_DISCORD_SMOKE_AGENT\n" +
    "  ANVIKA_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  ANVIKA_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  ANVIKA_DISCORD_SMOKE_POLL_MS\n" +
    "  ANVIKA_DISCORD_SMOKE_THREAD_BINDINGS_PATH"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.ANVIKA_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.ANVIKA_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.ANVIKA_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.ANVIKA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.ANVIKA_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.ANVIKA_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.ANVIKA_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.ANVIKA_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.ANVIKA_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.ANVIKA_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.ANVIKA_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.ANVIKA_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `anvika-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Anvika.`,
        diagnostics: {
          bindingCandidates: [
            {
