 ...
 * `anvika update`. Use it to verify the completion installation flow
...
\`anvika update\`, \`anvika doctor\`, and \`anvika onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Anvika Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`anvika completion --install\` later to enable.`));
    return;
  }
