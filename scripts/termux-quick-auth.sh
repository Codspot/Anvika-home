...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/anvika/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Anvika Re-Auth" -c "After getting key, run: ssh $SERVER '~/anvika/scripts/mobile-reauth.sh'" --id anvika-auth
        ;;
    *)
