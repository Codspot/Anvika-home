...
const shouldBuild = (deps) => {
  if (deps.env.ANVIKA_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.ANVIKA_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[anvika] ${message}\n`);
};

const runAnvika = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["anvika.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
