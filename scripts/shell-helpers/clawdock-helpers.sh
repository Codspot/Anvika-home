#!/usr/bin/env bash
# ClawDock - Docker helpers for Anvika
# Inspired by Simon Willison's "Running Anvika in Docker"
# https://til.simonwillison.net/llms/anvika-docker
#
# Installation:
...
# Common paths to check for Anvika
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/anvika"
  "${HOME}/workspace/anvika"
  "${HOME}/projects/anvika"
  "${HOME}/dev/anvika"
  "${HOME}/code/anvika"
  "${HOME}/src/anvika"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Anvika at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/anvika"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Anvika not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/anvika/anvika.git ~/anvika"
    echo "  cd ~/anvika && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^ANVIKA_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d anvika-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart anvika-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f anvika-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec anvika-gateway \
    bash -c 'echo "alias anvika=\"./anvika.mjs\"" > /tmp/.bashrc_anvika && bash --rcfile /tmp/.bashrc_anvika'
}
...
clawdock-cli() {
  _clawdock_compose run --rm anvika-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build anvika-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "ANVIKA_GATEWAY_TOKEN=$token" anvika-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" anvika-gateway \
    bash -c './anvika.mjs config set gateway.remote.token "$TOKEN" && ./anvika.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec anvika-gateway \
    bash -c "./anvika.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart anvika-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm anvika-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec anvika-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'anvika config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Anvika${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (anvika alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to anvika project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.anvika)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'anvika channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'anvika status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.anvika.ai${_CLR_RESET}"
  echo ""
}
