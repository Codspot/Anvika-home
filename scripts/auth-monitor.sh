...
#
# Suggested cron: */30 * * * * /home/admin/anvika/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Anvika notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., anvika-alerts)
...
STATE_FILE="$HOME/.anvika/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Anvika if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use anvika
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Anvika to $NOTIFY_PHONE..."
            anvika send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Anvika is down. Run: ssh l36 '~/anvika/scripts/mobile-reauth.sh'" "urgent"
    exit 1
