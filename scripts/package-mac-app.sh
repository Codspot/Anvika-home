...
# Build and bundle Anvika into a minimal .app we can open.
# Outputs to dist/Anvika.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Anvika.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Anvika"
BUNDLE_ID="${BUNDLE_ID:-ai.anvika.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/anvika/anvika/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Anvika/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AnvikaBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AnvikaGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Anvika"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Anvika"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Anvika"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Anvika" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Anvika/Resources/Anvika.icns" "$APP_ROOT/Contents/Resources/Anvika.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Anvika/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying AnvikaKit resources"
ANVIKAKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AnvikaKit_AnvikaKit.bundle"
if [ -d "$ANVIKAKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AnvikaKit_AnvikaKit.bundle"
  cp -R "$ANVIKAKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AnvikaKit_AnvikaKit.bundle"
else
  echo "WARN: AnvikaKit resource bundle not found at $ANVIKAKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Anvika"
killall -q Anvika 2>/dev/null || true
