...
INSTALL_URL="${ANVIKA_INSTALL_URL:-https://anvika.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${ANVIKA_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ANVIKA_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="anvika"
PACKAGE_NAME="${ANVIKA_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ANVIKA_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ANVIKA_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${ANVIKA_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
