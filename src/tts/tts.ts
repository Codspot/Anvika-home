...
export function resolveTtsConfig(cfg: AnvikaConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.ANVIKA_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: AnvikaConfig): string | undefined {
  ...
  for (const provider of providers) {
    ...
    try {
      if (provider === "edge") {
        ...
        const tempRoot = resolvePreferredAnvikaTmpDir();
