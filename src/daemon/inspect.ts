...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "anvika" | "clawdbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["anvika", "clawdbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["anvika_service_marker"];
  const kindKeys = ["anvika_service_kind"];
...
function isAnvikaGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.anvika.");
}

function isAnvikaGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("anvika-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isAnvikaGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("anvika gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if (marker === "anvika" && isAnvikaGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "anvika" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (marker === "anvika" && isAnvikaGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isAnvikaGatewayTaskName(name)) {
        continue;
      }
