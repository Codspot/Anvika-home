...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `anvika-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      ANVIKA_LAUNCHD_LABEL: `ai.anvika.launchd-int-${testId}`,
      ANVIKA_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
