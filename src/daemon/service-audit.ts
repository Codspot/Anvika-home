...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.ANVIKA_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service ANVIKA_GATEWAY_TOKEN does not match gateway.auth.token in anvika.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `anvika gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
