...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ANVIKA_GATEWAY_PORT).toBe("18789");
    expect(env.ANVIKA_GATEWAY_TOKEN).toBe("secret");
    expect(env.ANVIKA_SERVICE_MARKER).toBe("anvika");
    expect(env.ANVIKA_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ANVIKA_SERVICE_VERSION).toBe("string");
    expect(env.ANVIKA_SYSTEMD_UNIT).toBe("anvika-gateway.service");
    if (process.platform === "darwin") {
      expect(env.ANVIKA_LAUNCHD_LABEL).toBe("ai.anvika.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ANVIKA_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ANVIKA_SYSTEMD_UNIT).toBe("anvika-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.ANVIKA_LAUNCHD_LABEL).toBe("ai.anvika.work");
    }
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".anvika"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", ANVIKA_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".anvika-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", ANVIKA_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".anvika"));
  });

  it("uses ANVIKA_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", ANVIKA_STATE_DIR: "/var/lib/anvika" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/anvika"));
  });

  it("expands ~ in ANVIKA_STATE_DIR", () => {
    const env = { HOME: "/Users/test", ANVIKA_STATE_DIR: "~/anvika-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/anvika-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { ANVIKA_STATE_DIR: "C:\\State\\anvika" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\anvika");
  });
});
