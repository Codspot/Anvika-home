...
declare const __ANVIKA_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "anvika";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["ANVIKA_VERSION"],
      env["ANVIKA_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Anvika version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __ANVIKA_VERSION__ === "string" && __ANVIKA_VERSION__) ||
  process.env.ANVIKA_BUNDLED_VERSION ||
