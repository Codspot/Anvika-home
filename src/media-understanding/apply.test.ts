...
const TEMP_MEDIA_PREFIX = "anvika-media-";
...
function createGroqAudioConfig(): AnvikaConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): AnvikaConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): AnvikaConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: AnvikaConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredAnvikaTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        ANVIKA_AGENT_DIR: isolatedAgentDir,
