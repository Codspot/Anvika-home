...
describe("canvas host state dir defaults", () => {
  it("uses ANVIKA_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("anvika-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Anvika Canvas");
      } finally {
        await handler.close();
      }
    });
