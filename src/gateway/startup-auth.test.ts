...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: AnvikaConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: AnvikaConfig) {
  ...
  async function expectNoTokenGeneration(cfg: AnvikaConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  it("throws when hooks token reuses gateway token resolved from env", async () => {
    await expect(
      ensureGatewayStartupAuth({
        ...
        env: {
          ANVIKA_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
