...
const connectNodeClient = async (params: {
  ...
  const token = process.env.ANVIKA_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("ANVIKA_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".anvika", "restart-sentinel.json");
...
describe("gateway node command allowlist", () => {
  ...
  test("rejects reconnect metadata spoof for paired node devices", async () => {
    ...
    const deviceIdentityPath = path.join(
      os.tmpdir(),
      `anvika-spoof-test-device-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
