...
import type { AnvikaConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: AnvikaConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: AnvikaConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: AnvikaConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: AnvikaConfig) => void | Promise<void>;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: AnvikaConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: AnvikaConfig) => {
