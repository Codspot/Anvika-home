...
let lastCreateAnvikaToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/anvika-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateAnvikaToolsContext?.agentTo,
          agentThreadId: lastCreateAnvikaToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createAnvikaTools: (ctx: Record<string, unknown>) => {
      lastCreateAnvikaToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateAnvikaToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-anvika-message-to": "channel:24514",
        "x-anvika-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
