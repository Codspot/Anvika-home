    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "anvika", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-anvika-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-anvika-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "anvika", messages: [{ role: "user", content: "hi" }] },
          {
            "x-anvika-agent-id": "beta",
            "x-anvika-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from Anvika.");
      }
