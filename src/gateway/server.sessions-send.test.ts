...
const { createAnvikaTools } = await import("../agents/anvika-tools.js");
...
type SessionSendTool = ReturnType<typeof createAnvikaTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createAnvikaTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["ANVIKA_GATEWAY_PORT", "ANVIKA_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.ANVIKA_GATEWAY_PORT = String(gatewayPort);
  process.env.ANVIKA_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("ANVIKA_CONFIG_PATH missing in gateway test environment");
      }
