...
export function resolveHooksConfig(cfg: AnvikaConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: AnvikaConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-anvika-token"] === "string"
      ? req.headers["x-anvika-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
