...
import { resolveAnvikaAgentDir } from "../agents/agent-paths.js";
...
import { ensureAnvikaModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, AnvikaConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.ANVIKA_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.ANVIKA_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ANVIKA_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.ANVIKA_LIVE_GATEWAY_PROVIDERS);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.ANVIKA_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.ANVIKA_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<AnvikaConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.ANVIKA_CONFIG_PATH,
    token: process.env.ANVIKA_GATEWAY_TOKEN,
    skipChannels: process.env.ANVIKA_SKIP_CHANNELS,
    skipGmail: process.env.ANVIKA_SKIP_GMAIL_WATCHER,
    skipCron: process.env.ANVIKA_SKIP_CRON,
    skipCanvas: process.env.ANVIKA_SKIP_CANVAS_HOST,
    agentDir: process.env.ANVIKA_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.ANVIKA_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveAnvikaAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-live-state-"));
  process.env.ANVIKA_STATE_DIR = tempStateDir;
  ...
  process.env.ANVIKA_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.anvika-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveAnvikaAgentDir();
  const sanitizedCfg: AnvikaConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-live-"));
  const tempConfigPath = path.join(tempDir, "anvika.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.ANVIKA_CONFIG_PATH = tempConfigPath;

  await ensureAnvikaModelsJson(nextCfg);
  ...
  try {
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "Anvika live tool probe (local, safe): " +
    ...
    process.env.ANVIKA_CONFIG_PATH = previous.configPath;
    process.env.ANVIKA_GATEWAY_TOKEN = previous.token;
    process.env.ANVIKA_SKIP_CHANNELS = previous.skipChannels;
    process.env.ANVIKA_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.ANVIKA_SKIP_CRON = previous.skipCron;
    process.env.ANVIKA_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.ANVIKA_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ANVIKA_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureAnvikaModelsJson(cfg);
      ...
      const rawModels = process.env.ANVIKA_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via ANVIKA_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.anvika-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
