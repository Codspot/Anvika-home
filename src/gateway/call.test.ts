...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("anvika doctor --fix");
  });
...
describe("callGateway url override auth requirements", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ANVIKA_GATEWAY_TOKEN", "ANVIKA_GATEWAY_PASSWORD"]);
    resetGatewayCallMocks();
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.ANVIKA_GATEWAY_TOKEN = "env-token";
    process.env.ANVIKA_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      label: "password",
      authKey: "password",
      envKey: "ANVIKA_GATEWAY_PASSWORD",
      envValue: "from-env",
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ANVIKA_GATEWAY_PASSWORD", "ANVIKA_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.ANVIKA_GATEWAY_PASSWORD = envPassword;
    }
