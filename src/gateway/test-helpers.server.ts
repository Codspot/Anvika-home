...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "ANVIKA_BUNDLED_PLUGINS_DIR",
  "ANVIKA_SKIP_CHANNELS",
  "ANVIKA_SKIP_PROVIDERS",
  "ANVIKA_SKIP_CRON",
  "ANVIKA_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.ANVIKA_SKIP_PROVIDERS = "1";
  process.env.ANVIKA_SKIP_CRON = "1";
  process.env.ANVIKA_TEST_MINIMAL_GATEWAY = "1";
  process.env.ANVIKA_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "anvika-test-no-bundled-extensions")
    : "anvika-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "anvika-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".anvika-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__anvikaTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__anvikaTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["ANVIKA_GATEWAY_TOKEN"]);
  const prev = process.env.ANVIKA_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ANVIKA_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.ANVIKA_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.ANVIKA_GATEWAY_PASSWORD;
