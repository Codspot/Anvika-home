...
import { createAnvikaTools } from "../agents/anvika-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-anvika-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-anvika-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-anvika-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-anvika-thread-id")?.trim() || undefined;
  ...
  const allTools = createAnvikaTools({
