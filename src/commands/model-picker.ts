...
type PromptDefaultModelResult = { model?: string; config?: AnvikaConfig };
...
function resolveConfiguredModelRaw(cfg: AnvikaConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: AnvikaConfig): string[] {
...
export function applyPrimaryModel(cfg: AnvikaConfig, model: string): AnvikaConfig {
...
export function applyModelAllowlist(cfg: AnvikaConfig, models: string[]): AnvikaConfig {
  const defaults = cfg.agents?.defaults;
