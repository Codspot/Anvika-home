...
type EnvSnapshot = {
  HOME?: string;
  ANVIKA_HOME?: string;
  ANVIKA_STATE_DIR?: string;
  ANVIKA_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    ANVIKA_HOME: process.env.ANVIKA_HOME,
    ANVIKA_STATE_DIR: process.env.ANVIKA_STATE_DIR,
    ANVIKA_OAUTH_DIR: process.env.ANVIKA_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: AnvikaConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: AnvikaConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "anvika-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.ANVIKA_HOME = tempHome;
    process.env.ANVIKA_STATE_DIR = path.join(tempHome, ".anvika");
    delete process.env.ANVIKA_OAUTH_DIR;
    fs.mkdirSync(process.env.ANVIKA_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when ANVIKA_OAUTH_DIR is explicitly configured", async () => {
    process.env.ANVIKA_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: AnvikaConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints anvika-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: AnvikaConfig = {};
    setupSessionState(cfg, process.env, process.env.HOME ?? "");
    ...
    expect(text).toMatch(/anvika sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/anvika sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /anvika sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
