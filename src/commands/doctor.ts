...
function resolveMode(cfg: AnvikaConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveAnvikaPackageRoot({
  ...
  let cfg: AnvikaConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("anvika configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("anvika config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("anvika setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("anvika doctor --fix")}" to apply changes.`);
  }
