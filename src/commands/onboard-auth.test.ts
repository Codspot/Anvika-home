...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under ANVIKA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("anvika-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-oauth-sync-"));
    ...
    process.env.ANVIKA_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-oauth-nosync-"));
    process.env.ANVIKA_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside ANVIKA_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-oauth-external-"));
    process.env.ANVIKA_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* ANVIKA_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ANVIKA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("anvika-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
