...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAnvikaAgentDir: vi.fn().mockReturnValue("/tmp/anvika-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/anvika-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/anvika-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveAnvikaAgentDir: mocks.resolveAnvikaAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/anvika-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveAnvikaAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/anvika-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/anvika-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/anvika-agent-custom");
