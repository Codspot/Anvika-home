...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAnvikaAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("anvika models auth setup-token")}\` or \`${formatCliCommand("anvika configure")}\`.`
          : `Run \`${formatCliCommand("anvika configure")}\` or set an API key env var.`;
