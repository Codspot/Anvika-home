...
export async function loadValidConfigOrThrow(): Promise<AnvikaConfig> {
...
export async function updateConfig(
  mutator: (cfg: AnvikaConfig) => AnvikaConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: AnvikaConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AnvikaConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("anvika agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
