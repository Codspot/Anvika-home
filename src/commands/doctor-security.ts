...
import type { AnvikaConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: AnvikaConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("anvika security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.anvika/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("anvika approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.anvika.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("anvika configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("anvika config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("anvika doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("anvika config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('anvika config set session.dmScope "per-channel-peer"') +
