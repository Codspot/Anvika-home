...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("anvika pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelOnboardingDmPolicy) => {
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`anvika pairing approve ${policy.channel} <code>`)}`,
      ...
      if (adapter) {
        await prompter.note(
          `${channel} plugin not available (continuing with onboarding). If the channel still doesn't work after setup, run \`${formatCliCommand(
            "anvika plugins list",
          )}\` and \`${formatCliCommand("anvika plugins enable " + channel)}\`, then restart the gateway.`,
          "Channel setup",
        );
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("anvika channels add")}\``,
        },
      ],
