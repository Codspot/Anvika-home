...
function stripUnknownConfigKeys(config: AnvikaConfig): {
  config: AnvikaConfig;
  removed: string[];
} {
  const parsed = AnvikaSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: AnvikaConfig) {
...
export function collectMissingDefaultAccountBindingWarnings(cfg: AnvikaConfig): string[] {
...
function scanTelegramAllowFromUsernameEntries(cfg: AnvikaConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: AnvikaConfig): Promise<{
  config: AnvikaConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: AnvikaConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: AnvikaConfig): {
  config: AnvikaConfig;
  changes: string[];
} {
...
function scanMutableAllowlistEntries(cfg: AnvikaConfig): MutableAllowlistHit[] {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: AnvikaConfig): {
  config: AnvikaConfig;
  changes: string[];
} {
...
async function maybeRepairAllowlistPolicyAllowFrom(cfg: AnvikaConfig): Promise<{
  config: AnvikaConfig;
  changes: string[];
}> {
  const channels = cfg.channels;
 ...
 * file support.
 */
function detectEmptyAllowlistPolicy(cfg: AnvikaConfig): string[] {
  const channels = cfg.channels;
  if (!channels || typeof channels !== "object") {
    return [];
  }
  ...
  const checkAccount = (
    ...
    if (dmPolicy === "allowlist" && !hasAllowFromEntries(effectiveAllowFrom)) {
      warnings.push(
        `- ${prefix}.dmPolicy is "allowlist" but allowFrom is empty — all DMs will be blocked. Add sender IDs to ${prefix}.allowFrom, or run "${formatCliCommand("anvika doctor --fix")}" to auto-migrate from pairing store when entries exist.`,
      );
    }
...
function collectExecSafeBinScopes(cfg: AnvikaConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: AnvikaConfig): ExecSafeBinCoverageHit[] {
...
function scanExecSafeBinTrustedDirHints(cfg: AnvikaConfig): ExecSafeBinTrustedDirHintHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: AnvikaConfig): {
  config: AnvikaConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: AnvikaConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: AnvikaConfig): {
  config: AnvikaConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".anvika");
  const targetPath = path.join(targetDir, "anvika.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("anvika doctor --fix")}" to apply compatibility migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("anvika doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (shouldRepair) {
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("anvika doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("anvika doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("anvika doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (toolsBySenderHits.length > 0) {
      ...
      note(
        [
          ...
          `- Run "${formatCliCommand("anvika doctor --fix")}" to migrate legacy keys to typed id: entries.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (safeBinCoverage.length > 0) {
      ...
      lines.push(
        `- Run "${formatCliCommand("anvika doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`,
      );
      note(lines.join("\n"), "Doctor warnings");
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "anvika doctor --fix" to remove these keys.');
    }
  }
