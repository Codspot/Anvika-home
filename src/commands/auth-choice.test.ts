...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    ...
    const env = await setupAuthTestEnv("anvika-auth-");
      ...
      config: {
        secrets: {
          providers: {
            filemain: {
              source: "file",
              path: "/tmp/anvika-missing-secrets.json",
              mode: "json",
            },
          },
        },
  ...
  it("ignores legacy LiteLLM oauth profiles when selecting litellm-api-key", async () => {
    ...
    const authProfilePath = authProfilePathForAgent(requireAnvikaAgentDir());
    await fs.writeFile(
      authProfilePath,
      JSON.stringify(
        {
