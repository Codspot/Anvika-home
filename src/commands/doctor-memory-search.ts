 ...
 * Runs as part of `anvika doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("anvika config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("anvika memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("anvika configure --section model")}`,
        `- To disable: ${formatCliCommand("anvika config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("anvika memory status --deep")}`,
      ].join("\n"),
      "Memory search",
