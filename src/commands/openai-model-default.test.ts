...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: AnvikaConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: AnvikaConfig) => { changed: boolean; next: AnvikaConfig };
  defaultModel: string;
  overrideConfig: AnvikaConfig;
  alreadyDefaultConfig: AnvikaConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: AnvikaConfig) => config,
      applyDefaultConfig: (config: AnvikaConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: AnvikaConfig = { agents: { defaults: {} } };
