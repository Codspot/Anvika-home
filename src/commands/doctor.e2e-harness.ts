...
export const resolveAnvikaPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadAnvikaPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveAnvikaPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ANVIKA_STATE_DIR;
  originalUpdateInProgress = process.env.ANVIKA_UPDATE_IN_PROGRESS;
  process.env.ANVIKA_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "anvika-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.ANVIKA_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.ANVIKA_UPDATE_IN_PROGRESS;
  } else {
    process.env.ANVIKA_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
