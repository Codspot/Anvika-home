...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ANVIKA_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("ANVIKA_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ANVIKA_GATEWAY_TOKEN");
    expect(message).not.toContain("ANVIKA_GATEWAY_PASSWORD");
  });
