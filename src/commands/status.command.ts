...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Anvika status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`anvika devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("anvika devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("anvika devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("anvika security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("anvika security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.anvika.ai/faq");
  runtime.log("Troubleshooting: https://docs.anvika.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("anvika status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("anvika logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("anvika status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("anvika gateway probe")}`);
  }
}
