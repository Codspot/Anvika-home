...
beforeAll(() => {
  envSnapshot = captureEnv(["ANVIKA_PROFILE"]);
  process.env.ANVIKA_PROFILE = "isolated";
});
...
vi.mock("../infra/anvika-root.js", () => ({
  resolveAnvikaPackageRoot: vi.fn().mockResolvedValue("/tmp/anvika"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/anvika/pnpm-lock.yaml",
      markerPath: "/tmp/anvika/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.anvika.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.anvika.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("Anvika status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("anvika status --all") ||
          l.includes("anvika --profile isolated status --all") ||
          l.includes("anvika status --all") ||
          l.includes("anvika --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.ANVIKA_GATEWAY_TOKEN = "abcd1234";
    try {
