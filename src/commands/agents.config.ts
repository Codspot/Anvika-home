...
function resolveAgentName(cfg: AnvikaConfig, agentId: string) {
...
function resolveAgentModel(cfg: AnvikaConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AnvikaConfig): AgentSummary[] {
