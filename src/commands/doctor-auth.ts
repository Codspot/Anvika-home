...
function pruneAuthProfiles(
  cfg: AnvikaConfig,
  profileIds: Set<string>,
): { next: AnvikaConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("anvika models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("anvika models auth setup-token")} or ${formatCliCommand(
      "anvika configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "anvika models auth login --provider openai-codex",
    )} or ${formatCliCommand("anvika configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("anvika configure")}\` or \`${formatCliCommand("anvika onboard")}\`.`;
}
