...
const SEARCH_TOOL = "https://docs.anvika.ai/mcp.SearchAnvika";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.anvika.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('anvika docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.anvika.ai/");
      runtime.log(`Search: ${formatCliCommand('anvika docs "your query"')}`);
    }
    return;
  }
