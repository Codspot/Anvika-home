...
export function normalizeCompatibilityConfigValues(cfg: AnvikaConfig): {
  config: AnvikaConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: AnvikaConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as AnvikaConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as AnvikaConfig["browser"],
    };
    changes.push(
