  ...
  while (true) {
    ...
    if (source === "env") {
      ...
      await params.prompter.note(
        `Validated environment variable ${envVar}. Anvika will store a reference, not the key value.`,
        "Reference validated",
      );
    ...
    try {
      ...
      await params.prompter.note(
        `Validated ${providerEntry.source} reference ${selectedProvider}:${id}. Anvika will store a reference, not the key value.`,
        "Reference validated",
      );
      return { ref, resolvedValue };
    } catch (error) {
    ...
    options: [
      {
        ...
        hint: "Stores the key directly in Anvika config",
      },
      {
        value: "ref",
