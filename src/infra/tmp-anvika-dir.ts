...
export const POSIX_ANVIKA_TMP_DIR = "/tmp/anvika";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredAnvikaTmpDirOptions = {
...
export function resolvePreferredAnvikaTmpDir(
  options: ResolvePreferredAnvikaTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "anvika" : `anvika-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[anvika] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Anvika temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Anvika temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_ANVIKA_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_ANVIKA_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_ANVIKA_TMP_DIR)) {
      return POSIX_ANVIKA_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_ANVIKA_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_ANVIKA_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_ANVIKA_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_ANVIKA_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_ANVIKA_TMP_DIR;
