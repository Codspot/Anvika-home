...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __anvikaDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__anvikaDiagnosticEventsState) {
    globalStore.__anvikaDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__anvikaDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: AnvikaConfig): boolean {
