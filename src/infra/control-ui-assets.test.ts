...
vi.mock("./anvika-root.js", () => ({
  resolveAnvikaPackageRoot: vi.fn(async () => null),
  resolveAnvikaPackageRootSync: vi.fn(() => null),
}));
...
let anvikaRoot: typeof import("./anvika-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    anvikaRoot = await import("./anvika-root.js");
  });
  ...
  it("uses resolveAnvikaPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/anvika");
    (
      anvikaRoot.resolveAnvikaPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/anvika"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "anvika" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "anvika.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-anvika");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/anvika-bundle");
    (
      anvikaRoot.resolveAnvikaPackageRootSync as unknown as ReturnType<typeof vi.fn>
