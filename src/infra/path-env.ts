...
function candidateBinDirs(opts: EnsureAnvikaPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `anvika` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "anvika");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.ANVIKA_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "anvika"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `anvika` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAnvikaCliOnPath(opts: EnsureAnvikaPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ANVIKA_PATH_BOOTSTRAPPED)) {
    return;
  }
