...
const DEFAULT_PACKAGE_NAME = "anvika";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "anvika.mjs");
    ...
    const doctorStep = await runStep(
      step("anvika doctor", doctorArgv, gitRoot, { ANVIKA_UPDATE_IN_PROGRESS: "1" }),
    );
