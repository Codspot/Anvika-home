...
import { POSIX_ANVIKA_TMP_DIR, resolvePreferredAnvikaTmpDir } from "./tmp-anvika-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredAnvikaTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `anvika-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const wrappedLstatSync = vi.fn((target: string) => {
    if (target === POSIX_ANVIKA_TMP_DIR) {
  ...
  const resolved = resolvePreferredAnvikaTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredAnvikaTmpDir", () => {
  it("prefers /tmp/anvika when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_ANVIKA_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/anvika when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_ANVIKA_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/anvika when /tmp/anvika is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/anvika when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/anvika is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/anvika is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/anvika is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Anvika temp dir/);
  });
