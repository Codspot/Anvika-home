...
let ensureAnvikaCliOnPath: typeof import("./path-env.js").ensureAnvikaCliOnPath;

describe("ensureAnvikaCliOnPath", () => {
  const envKeys = [
    "PATH",
    "ANVIKA_PATH_BOOTSTRAPPED",
    "ANVIKA_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureAnvikaCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling anvika exists", () => {
    const tmp = abs("/tmp/anvika-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "anvika");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.ANVIKA_PATH_BOOTSTRAPPED;

    ensureAnvikaCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.ANVIKA_PATH_BOOTSTRAPPED = "1";
    ensureAnvikaCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/anvika-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "anvika");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/anvika-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "anvika");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "anvika");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/anvika-path/case-linuxbrew");
