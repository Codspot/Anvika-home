...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ANVIKA_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.ANVIKA_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveAnvikaPackageRoot } = await import("./anvika-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveAnvikaPackageRoot).mockResolvedValue("/opt/anvika");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/anvika/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
