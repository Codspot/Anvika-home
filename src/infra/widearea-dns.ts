...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ANVIKA_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*anvika-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "anvika";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "anvika");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "anvika-gw");
  ...
  records.push(`_anvika-gw._tcp IN PTR ${instanceLabel}._anvika-gw._tcp`);
  records.push(`${instanceLabel}._anvika-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._anvika-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; anvika-content-hash: ${contentHash}\n${contentBody}`;
}
