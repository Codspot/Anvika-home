...
import { resolvePreferredAnvikaTmpDir } from "../tmp-anvika-dir.js";
...
const telegramChunkConfig: AnvikaConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: AnvikaConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes Anvika tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredAnvikaTmpDir()]),
      }),
    );
  });

  it("includes Anvika tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes Anvika tmp root in whatsapp mediaLocalRoots", async () => {
  ...
  it("includes Anvika tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: AnvikaConfig = { channels: { signal: { mediaMaxMb: 2 } } };
