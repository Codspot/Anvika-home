...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred Anvika tmp root", async () => {
    const tmpRoot = resolvePreferredAnvikaTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideAnvika = path.join(os.tmpdir(), "outside-anvika", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
