...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AnvikaConfig | null = null;
  ...
  get(key: string, cfg: AnvikaConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AnvikaConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: AnvikaConfig): void {
