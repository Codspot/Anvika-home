...
describe("resolveEffectiveHomeDir", () => {
  it("prefers ANVIKA_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands ANVIKA_HOME when set to ~", () => {
    const env = {
      ANVIKA_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for ANVIKA_HOME", () => {
    const result = resolveRequiredHomeDir(
      { ANVIKA_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when ANVIKA_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ ANVIKA_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { ANVIKA_HOME: "/srv/anvika-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/anvika-home")}/x`);
  });
