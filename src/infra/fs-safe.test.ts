...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("anvika-fs-safe-");
  ...
  it("blocks traversal outside root", async () => {
    const root = await tempDirs.make("anvika-fs-safe-root-");
    const outside = await tempDirs.make("anvika-fs-safe-outside-");
