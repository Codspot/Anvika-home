...
export function triggerAnvikaRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.ANVIKA_SYSTEMD_UNIT,
        process.env.ANVIKA_PROFILE,
      );
  ...
  const label =
    process.env.ANVIKA_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ANVIKA_PROFILE);
