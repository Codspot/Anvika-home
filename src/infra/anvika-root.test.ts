...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__anvika_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "anvika-root");
...
describe("resolveAnvikaPackageRoot", () => {
  let resolveAnvikaPackageRoot: typeof import("./anvika-root.js").resolveAnvikaPackageRoot;
  let resolveAnvikaPackageRootSync: typeof import("./anvika-root.js").resolveAnvikaPackageRootSync;

  beforeAll(async () => {
    ({ resolveAnvikaPackageRoot, resolveAnvikaPackageRootSync } =
      await import("./anvika-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "anvika");
    const pkgRoot = path.join(project, "node_modules", "anvika");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "anvika" }));

    expect(resolveAnvikaPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "anvika");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "anvika.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "anvika" }));

    expect(resolveAnvikaPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveAnvikaPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-anvika package roots", async () => {
    const pkgRoot = fx("not-anvika");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-anvika" }));

    expect(resolveAnvikaPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveAnvikaPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveAnvikaPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
