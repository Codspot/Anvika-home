...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: AnvikaConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: AnvikaConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: AnvikaConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: AnvikaConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: AnvikaConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: AnvikaConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: AnvikaConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
