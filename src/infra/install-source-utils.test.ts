...
const TEMP_DIR_PREFIX = "anvika-install-source-utils-";
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("anvika-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-anvika-archive.tgz");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    const archivePath = path.join(cwd, "anvika-plugin-1.2.3.tgz");
    await fs.writeFile(archivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "anvika-plugin@1.2.3",
          name: "anvika-plugin",
          version: "1.2.3",
          filename: "anvika-plugin-1.2.3.tgz",
    ...
    const result = await runPack("anvika-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "anvika-plugin",
        version: "1.2.3",
        resolvedSpec: "anvika-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "anvika-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "anvika-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\nanvika-plugin-1.2.3.tgz\n",
    });
  ...
  it("falls back to archive detected in cwd when npm pack stdout is empty", async () => {
    const cwd = await createTempDir("anvika-install-source-utils-");
    const archivePath = path.join(cwd, "anvika-plugin-1.2.3.tgz");
    await fs.writeFile(archivePath, "", "utf-8");
    runCommandWithTimeoutMock.mockResolvedValue({
      stdout: " \n\n",
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@anvika/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@anvika/whatsapp", cwd);

    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("Package not found on npm");
      expect(result.error).toContain("@anvika/whatsapp");
      expect(result.error).toContain("docs.anvika.ai/tools/plugin");
    }
  });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("anvika-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "anvika-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@anvika/plugin-demo@2.0.0",
            filename: "anvika-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@anvika/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "anvika-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@anvika/plugin-demo@2.0.0",
      },
    });
  });
