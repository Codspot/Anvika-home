...
import { resolveAnvikaPackageRoot, resolveAnvikaPackageRootSync } from "./anvika-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveAnvikaPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "anvika" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveAnvikaPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
