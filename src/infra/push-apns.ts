...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.ANVIKA_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.ANVIKA_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set ANVIKA_APNS_TEAM_ID and ANVIKA_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.ANVIKA_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.ANVIKA_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.ANVIKA_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set ANVIKA_APNS_PRIVATE_KEY_P8 or ANVIKA_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading ANVIKA_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createAnvikaPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    anvika: createAnvikaPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
