...
function resolveVoiceTtsConfig(params: { cfg: AnvikaConfig; override?: TtsConfig }): {
  cfg: AnvikaConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredAnvikaTmpDir(), "discord-voice-"));
