  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/anvika",
      },
    },
    session: { store: "/tmp/anvika-sessions.json" },
...
describe("discord tool result dispatch", () => {
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\banvika\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "anvika: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "Anvika" },
          },
        },
        eventPatch: {
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/anvika" },
      session: { store: "/tmp/anvika-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
