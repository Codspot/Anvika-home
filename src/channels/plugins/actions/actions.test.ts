...
function telegramCfg(): AnvikaConfig {
  return { channels: { telegram: { botToken: "tok" } } } as AnvikaConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: AnvikaConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as AnvikaConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as AnvikaConfig;
...
function createSignalAccountOverrideCfg(): AnvikaConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as AnvikaConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as AnvikaConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as AnvikaConfig,
        accountId: undefined,
        params: {
