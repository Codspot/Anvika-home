...
import { discoverAnvikaPlugins } from "../../plugins/discovery.js";
import type { AnvikaPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, AnvikaPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["ANVIKA_PLUGIN_CATALOG_PATHS", "ANVIKA_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<AnvikaPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: AnvikaPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: AnvikaPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverAnvikaPlugins({ workspaceDir: options.workspaceDir });
