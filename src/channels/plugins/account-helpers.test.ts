...
function cfg(accounts?: Record<string, unknown> | null): AnvikaConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as AnvikaConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as AnvikaConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as AnvikaConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as AnvikaConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as AnvikaConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as AnvikaConfig)).toBe("default");
    });
  });
});
