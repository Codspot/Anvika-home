...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@anvika/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "anvika-catalog-"));
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: AnvikaConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
