...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: AnvikaConfig, policy: ChannelAccessPolicy) => AnvikaConfig;
  resolveAllowlist: (params: { cfg: AnvikaConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: AnvikaConfig; resolved: TResolved }) => AnvikaConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: AnvikaConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: AnvikaConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: AnvikaConfig, policy: ChannelAccessPolicy): AnvikaConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: AnvikaConfig, policy: ChannelAccessPolicy): AnvikaConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: AnvikaConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: AnvikaConfig; resolved: string[] }) => {
      calls.push("apply");
