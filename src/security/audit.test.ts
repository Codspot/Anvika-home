...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: AnvikaConfig, accountId: string | null | undefined) => unknown;
...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync(
      { ANVIKA_STATE_DIR: channelSecurityStateDir },
      async () => await fn(channelSecurityStateDir),
    );
  };

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-security-audit-"));
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.ANVIKA_GATEWAY_TOKEN;
    const prevPassword = process.env.ANVIKA_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.ANVIKA_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.ANVIKA_GATEWAY_PASSWORD;
      } else {
        process.env.ANVIKA_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/anvika-safe-bins"];
    const cfg: AnvikaConfig = {
      tools: {
        exec: {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    ...
    const configPath = path.join(stateDir, "anvika.json");
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("anvika-sbx-browser-old\nanvika-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "anvika-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "anvika-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("anvika-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("anvika-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "anvika-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "anvika-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-anvika.json");
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): AnvikaConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: AnvikaConfig = { session: { dmScope: "main" } };
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.anvika/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: AnvikaConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.ANVIKA_GATEWAY_TOKEN;
    process.env.ANVIKA_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: AnvikaConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<AnvikaConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: AnvikaConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.anvika",
      configPath: "/Users/test/Dropbox/.anvika/anvika.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: AnvikaConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "anvika.json"),
      });
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@anvika/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@anvika/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  it("does not scan plugin code safety findings when deep audit is disabled", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        anvika: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.ANVIKA_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.ANVIKA_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
