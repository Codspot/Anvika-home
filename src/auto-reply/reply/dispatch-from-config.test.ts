...
const ttsMocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveTtsConfig: vi.fn((_cfg: AnvikaConfig) => ({ mode: "final" })),
  };
});
...
vi.mock("../../tts/tts.js", () => ({
  ...
  resolveTtsConfig: (cfg: AnvikaConfig) => ttsMocks.resolveTtsConfig(cfg),
}));
...
const emptyConfig = {} as AnvikaConfig;
...
describe("dispatchReplyFromConfig", () => {
  ...
  it("emits diagnostics when enabled", async () => {
    setNoAbort();
    const cfg = { diagnostics: { enabled: true } } as AnvikaConfig;
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
      Provider: "slack",
      Surface: "slack",
