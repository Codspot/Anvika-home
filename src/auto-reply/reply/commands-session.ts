...
import { scheduleGatewaySigusr1Restart, triggerAnvikaRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Anvika in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerAnvikaRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Anvika via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
