...
export function resolveConfiguredAcpBackendId(cfg: AnvikaConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: AnvikaConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(localPath)) {
      return `anvika plugins install ${localPath}`;
    }
    return "anvika plugins install @anvika/acpx";
  }
