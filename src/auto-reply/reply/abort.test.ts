...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop anvika!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@anvika_bot", { botUsername: "anvika_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@anvika_bot", { botUsername: "anvika_bot" })).toBe(true);
