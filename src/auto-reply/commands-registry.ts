...
export function isCommandEnabled(cfg: AnvikaConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AnvikaConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as AnvikaConfig),
...
export function getCommandDetection(_cfg?: AnvikaConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AnvikaConfig) {
