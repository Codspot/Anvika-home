...
import type { AnvikaConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredAnvikaTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "anvika.log"); // legacy single-file path

const LOG_PREFIX = "anvika";
...
function resolveSettings(): ResolvedSettings {
  let cfg: AnvikaConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => AnvikaConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.ANVIKA_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[anvika] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
