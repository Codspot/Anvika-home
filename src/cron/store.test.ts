...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ANVIKA_HOME for tilde expansion", () => {
    vi.stubEnv("ANVIKA_HOME", "/srv/anvika-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/anvika-home", "cron", "jobs.json"));
  });
});
