...
import type { AnvikaConfig, ConfigFileSnapshot } from "../config/types.anvika.js";
...
vi.mock("../infra/anvika-root.js", () => ({
  resolveAnvikaPackageRoot: vi.fn(),
}));
...
const { resolveAnvikaPackageRoot } = await import("../infra/anvika-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/anvika-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as AnvikaConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/anvika-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveAnvikaPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("anvika-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveAnvikaPackageRoot).mockClear();
    ...
    vi.mocked(resolveAnvikaPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("anvika");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/anvika-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "anvika-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Anvika update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as AnvikaConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveAnvikaPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("anvika-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ ANVIKA_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.ANVIKA_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("anvika-update-wizard");
    await withEnvAsync({ ANVIKA_GIT_DIR: tempDir }, async () => {
      setTty(true);
