...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Anvika and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["anvika update", "Update a source checkout (git)"],
        ["anvika update --channel beta", "Switch to beta channel (git + npm)"],
        ["anvika update --channel dev", "Switch to dev channel (git + npm)"],
        ["anvika update --tag beta", "One-off update to a dist-tag or version"],
        ["anvika update --dry-run", "Preview actions without changing anything"],
        ["anvika update --no-restart", "Update without restarting the service"],
        ["anvika update --json", "Output result as JSON"],
        ["anvika update --yes", "Non-interactive (accept downgrade prompts)"],
        ["anvika update wizard", "Interactive update wizard"],
        ["anvika --update", "Shorthand for anvika update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run anvika update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.anvika.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["anvika update status", "Show channel + version status."],
          ["anvika update status --json", "JSON output."],
          ["anvika update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.anvika.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
