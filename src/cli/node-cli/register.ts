    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "anvika node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["anvika node status", "Check node host service status."],
          ["anvika node install", "Install the node host service."],
          ["anvika node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.anvika.ai/cli/node")}\n`,
    );
