...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("anvika node install"), formatCliCommand("anvika node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("anvika node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    ANVIKA_LOG_PREFIX: baseEnv.ANVIKA_LOG_PREFIX ?? "node",
