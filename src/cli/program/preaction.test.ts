...
beforeEach(() => {
  ...
  originalHideBanner = process.env.ANVIKA_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.ANVIKA_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.ANVIKA_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "anvika", "status", "--debug"],
    });
  ...
  it("loads plugin registry for configure command", async () => {
    await runCommand({
      parseArgv: ["configure"],
      processArgv: ["node", "anvika", "configure"],
    });
  ...
  it("loads plugin registry for onboard command", async () => {
    await runCommand({
      parseArgv: ["onboard"],
      processArgv: ["node", "anvika", "onboard"],
    });
  ...
  it("loads plugin registry for agents command", async () => {
    await runCommand({
      parseArgv: ["agents"],
      processArgv: ["node", "anvika", "agents"],
    });
  ...
  it("skips config guard for doctor, completion, and secrets commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "anvika", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "anvika", "completion"],
    });
    await runCommand({
      parseArgv: ["secrets"],
      processArgv: ["node", "anvika", "secrets"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "anvika", "--version"],
    });
  ...
  it("hides banner when ANVIKA_HIDE_BANNER is truthy", async () => {
    process.env.ANVIKA_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "anvika", "status"],
    });
