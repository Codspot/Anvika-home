...
${formatHelpExamples([
  ['anvika agent --to +15555550123 --message "status update"', "Start a new session."],
  ['anvika agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'anvika agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'anvika agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['anvika agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'anvika agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.anvika.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.anvika.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['anvika agents set-identity --agent main --name "Anvika" --emoji "🦞"', "Set name + emoji."],
  ["anvika agents set-identity --agent main --avatar avatars/anvika.png", "Set avatar path."],
  [
    "anvika agents set-identity --workspace ~/.anvika/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "anvika agents set-identity --identity-file ~/.anvika/workspace/IDENTITY.md --agent main",
