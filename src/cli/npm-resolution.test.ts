...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@anvika/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@anvika/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@anvika/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@anvika/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @anvika/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@anvika/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@anvika/plugin-alpha@1.2.3",
        installPath: "/tmp/anvika/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@anvika/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @anvika/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@anvika/plugin-alpha@latest",
      true,
      "/tmp/anvika/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@anvika/plugin-alpha@latest",
      installPath: "/tmp/anvika/extensions/alpha",
      version: "1.2.3",
