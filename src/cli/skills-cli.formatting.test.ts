...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "anvika-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "anvika-bundled-skills-test-"));
    process.env.ANVIKA_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
