    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["anvika memory status", "Show index and provider status."],
          ["anvika memory index --force", "Force a full reindex."],
          ['anvika memory search --query "deployment notes"', "Search indexed memory entries."],
          ["anvika memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.anvika.ai/cli/memory")}\n`,
    );
