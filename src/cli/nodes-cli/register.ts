    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["anvika nodes status", "List known nodes with live status."],
          ["anvika nodes pairing pending", "Show pending node pairing requests."],
          ['anvika nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["anvika nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.anvika.ai/cli/nodes")}\n`,
    );
