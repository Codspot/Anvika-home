...
import { resolveAnvikaPackageRoot } from "../../infra/anvika-root.js";
...
const ANVIKA_REPO_URL = "https://github.com/anvika/anvika.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "anvika";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("anvika@")) {
    return trimmed.slice("anvika@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.ANVIKA_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveAnvikaPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", ANVIKA_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `ANVIKA_GIT_DIR points at a non-git directory: ${params.dir}. Set ANVIKA_GIT_DIR to an empty folder or an anvika checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`ANVIKA_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "anvika.mjs");
