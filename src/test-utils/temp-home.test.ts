...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("anvika-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.ANVIKA_STATE_DIR).toBe(path.join(tempHome.home, ".anvika"));
    ...
    expect(process.env.ANVIKA_STATE_DIR).toBe(previousStateDir);
