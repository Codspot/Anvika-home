...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => AnvikaConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("anvika-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): AnvikaConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
