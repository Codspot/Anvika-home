...
homepage: https://docs.anvika.ai/automation/hooks#command-logger
metadata:
  {
    "anvika":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.anvika/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.anvika/logs/commands.log ~/.anvika/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/anvika`:
   ```
   /home/username/.anvika/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
