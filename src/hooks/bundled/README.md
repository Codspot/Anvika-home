...
This directory contains hooks that ship with Anvika. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.anvika/anvika.json`:
...
2. Restart gateway: `pkill -9 -f 'anvika.*gateway' && pnpm anvika gateway`
3. Enable the hook: `anvika hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.anvika.ai/automation/hooks
