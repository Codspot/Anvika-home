...
const fixtureRoot = path.join(os.tmpdir(), `anvika-hook-install-${randomUUID()}`);
let tempDirIndex = 0;
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@anvika/test-hooks",
        version: "0.0.1",
        anvika: { hooks: ["./hooks/one-hook"] },
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'metadata: {"anvika":{"events":["command:new"]}}',
        "---",
        "",
        "# One Hook",
      ].join("\n"),
  ...
  it("rejects hook pack entries that traverse outside package directory", async () => {
    ...
    expect(result.error).toContain("anvika.hooks entry escapes package directory");
  });
  ...
  it("rejects hook pack entries that escape via symlink", async () => {
    ...
    expect(result.error).toContain("anvika.hooks entry resolves outside package directory");
  });
});
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@anvika/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@anvika/test-hooks@0.0.1",
    });
