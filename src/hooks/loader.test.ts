...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ANVIKA_BUNDLED_HOOKS_DIR"]);
    process.env.ANVIKA_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
