...
describe("handleSystemRunInvoke mac app exec host routing", () => {
  ...
  async function withTempApprovalsHome<T>(params: {
    ...
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "anvika-exec-approvals-"));
    const previousAnvikaHome = process.env.ANVIKA_HOME;
    process.env.ANVIKA_HOME = tempHome;
    saveExecApprovals(params.approvals);
    try {
      return await params.run({ tempHome });
    } finally {
      if (previousAnvikaHome === undefined) {
        delete process.env.ANVIKA_HOME;
      } else {
        process.env.ANVIKA_HOME = previousAnvikaHome;
      }
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "anvika-approval-cwd-link-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "anvika-approval-cwd-parent-link-"));
  ...
  it("uses canonical executable path for approval-based relative command execution", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "anvika-approval-cwd-real-"));
  ...
  it("denies ./sh wrapper spoof in allowlist on-miss mode before execution", async () => {
    const marker = path.join(os.tmpdir(), `anvika-wrapper-spoof-${process.pid}-${Date.now()}`);
    const runCommand = vi.fn(async () => {
  ...
  it("denies semicolon-chained shell payloads in allowlist mode without explicit approval", async () => {
    const payloads = ["anvika status; id", "anvika status; cat /etc/passwd"];
