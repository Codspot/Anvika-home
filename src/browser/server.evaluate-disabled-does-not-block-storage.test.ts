  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          anvika: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ANVIKA_GATEWAY_PORT;
    process.env.ANVIKA_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ANVIKA_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ANVIKA_GATEWAY_PASSWORD;
