...
const RELAY_TOKEN_CONTEXT = "anvika-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const ANVIKA_RELAY_BROWSER = "Anvika/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.ANVIKA_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or ANVIKA_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedAnvikaRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === ANVIKA_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
