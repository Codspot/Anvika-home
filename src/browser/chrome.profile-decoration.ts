...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".anvika-profile-decorated");
}
 ...
 */
export function decorateAnvikaProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ANVIKA_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ANVIKA_BROWSER_COLOR).toUpperCase();
