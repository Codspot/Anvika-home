import type { BrowserConfig, BrowserProfileConfig, AnvikaConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_ANVIKA_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "anvika" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_ANVIKA_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "anvika" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ANVIKA_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ANVIKA_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Anvika browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for anvika/anvika CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: AnvikaConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ANVIKA_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "anvika";
