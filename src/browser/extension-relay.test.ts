...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "ANVIKA_EXTENSION_RELAY_RECONNECT_GRACE_MS",
      "ANVIKA_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS",
    ]);
    process.env.ANVIKA_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
    delete process.env.ANVIKA_EXTENSION_RELAY_RECONNECT_GRACE_MS;
    delete process.env.ANVIKA_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS;
  });
  ...
  it("uses relay-scoped token only for known relay ports", async () => {
    ...
    expect(Object.keys(headers)).toContain("x-anvika-relay-token");
    expect(headers["x-anvika-relay-token"]).not.toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("allows CORS preflight from chrome-extension origins", async () => {
    ...
    const res = await fetch(`${cdpUrl}/json/version`, {
      ...
      headers: {
        ...
        "Access-Control-Request-Headers": "x-anvika-relay-token",
      },
    });
  ...
  it("closes CDP clients after reconnect grace when extension stays disconnected", async () => {
    process.env.ANVIKA_EXTENSION_RELAY_RECONNECT_GRACE_MS = "150";
  ...
  it("accepts extension websocket access with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(`ws://127.0.0.1:${port}/extension`)["x-anvika-relay-token"];
  ...
  it("accepts /json endpoints with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(cdpUrl)["x-anvika-relay-token"];
    expect(token).toBeTruthy();
  ...
  it("accepts raw gateway token for relay auth compatibility", async () => {
    ...
    const versionRes = await fetch(`${cdpUrl}/json/version`, {
      headers: { "x-anvika-relay-token": TEST_GATEWAY_TOKEN },
    });
    expect(versionRes.status).toBe(200);
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    const fakeRelay = createServer((req, res) => {
      if (req.url?.startsWith("/json/version")) {
        const header = req.headers["x-anvika-relay-token"];
        ...
        res.end(JSON.stringify({ Browser: "Anvika/extension-relay" }));
        return;
      }
  ...
  it("does not swallow EADDRINUSE when occupied port is not an anvika relay", async () => {
    const port = await getFreePort();
