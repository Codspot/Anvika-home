...
describe("extension-relay-auth", () => {
  ...
  beforeEach(() => {
    prevGatewayToken = process.env.ANVIKA_GATEWAY_TOKEN;
  ...
  it("accepts authenticated anvika relay probe responses", async () => {
    ...
    await withRelayServer(
      ...
      async ({ port }) => {
        ...
        const ok = await probeAuthenticatedAnvikaRelay({
          baseUrl: `http://127.0.0.1:${port}`,
          relayAuthHeader: "x-anvika-relay-token",
