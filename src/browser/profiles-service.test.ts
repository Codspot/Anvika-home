...
vi.mock("./chrome.js", () => ({
  resolveAnvikaUserDataDir: vi.fn(() => "/tmp/anvika-test/anvika/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveAnvikaUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "anvika-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveAnvikaUserDataDir).mockReturnValue(userDataDir);
