...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const anvika = resolveProfile(resolved, "anvika");
    expect(anvika?.driver).toBe("anvika");
    expect(anvika?.cdpPort).toBe(18800);
    expect(anvika?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from ANVIKA_GATEWAY_PORT when unset", () => {
    withEnv({ ANVIKA_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const anvika = resolveProfile(resolved, "anvika");
      expect(anvika?.cdpPort).toBe(19012);
      expect(anvika?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ ANVIKA_GATEWAY_PORT: undefined }, () => {
      ...
      const anvika = resolveProfile(resolved, "anvika");
      expect(anvika?.cdpPort).toBe(19022);
      expect(anvika?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "anvika");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        anvika: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("anvika");
  });
