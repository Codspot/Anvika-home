...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredAnvikaTmpDir: vi.fn(() => "/tmp/anvika"),
}));
vi.mock("../infra/tmp-anvika-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredAnvikaTmpDir.mockReturnValue("/tmp/anvika");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredAnvikaTmpDir.mockReturnValue("/tmp/anvika-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "anvika-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredAnvikaTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "anvika-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "anvika-preferred", "downloads")}${path.sep}`),
    );
  });
