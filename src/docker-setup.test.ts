...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "anvika-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ANVIKA_GATEWAY_TOKEN: "test-token",
    ANVIKA_CONFIG_DIR: join(sandbox.rootDir, "config"),
    ANVIKA_WORKSPACE_DIR: join(sandbox.rootDir, "anvika"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ANVIKA_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      ANVIKA_EXTRA_MOUNTS: undefined,
      ANVIKA_HOME_VOLUME: "anvika-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ANVIKA_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("ANVIKA_EXTRA_MOUNTS=");
    expect(envFile).toContain("ANVIKA_HOME_VOLUME=anvika-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("anvika-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("anvika-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg ANVIKA_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ANVIKA_CONFIG_DIR: configDir,
      ANVIKA_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("reuses existing config token when ANVIKA_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ANVIKA_GATEWAY_TOKEN=config-token-123");
  });

  it("rejects injected multiline ANVIKA_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ANVIKA_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ANVIKA_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid ANVIKA_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ANVIKA_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid ANVIKA_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ANVIKA_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ANVIKA_HOME_VOLUME must match");
  });
