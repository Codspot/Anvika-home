...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AnvikaPluginDefinition;
  register?: AnvikaPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AnvikaPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AnvikaPluginDefinition;
...
export function loadAnvikaPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverAnvikaPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "anvika/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "anvika/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: AnvikaPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as AnvikaPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
