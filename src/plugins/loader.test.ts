...
import { __testing, loadAnvikaPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `anvika-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.ANVIKA_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "anvika.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.ANVIKA_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadAnvikaPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadAnvikaPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.ANVIKA_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.ANVIKA_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadAnvikaPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadAnvikaPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadAnvikaPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.ANVIKA_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ ANVIKA_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
