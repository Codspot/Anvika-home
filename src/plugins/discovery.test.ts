...
import { discoverAnvikaPlugins } from "./discovery.js";

const tempDirs: string[] = [];

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `anvika-plugins-${randomUUID()}`);
  ...
  return await withEnvAsync(
    {
      ...
      ANVIKA_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
    },
    fn,
  );
}
...
describe("discoverAnvikaPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".anvika", "extensions");
    fs.mkdirSync(workspaceExt, { recursive: true });
    fs.writeFileSync(path.join(workspaceExt, "beta.ts"), "export default function () {}", "utf-8");

    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverAnvikaPlugins({ workspaceDir });
    });
  ...
  it("loads package extension packs", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "pack",
        anvika: { extensions: ["./src/one.ts", "./src/two.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        anvika: { extensions: ["./src/index.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverAnvikaPlugins({ extraPaths: [packDir] });
    });
  ...
  it("rejects package extension entries that escape via symlink", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@anvika/pack",
        anvika: { extensions: ["./linked/escape.ts"] },
      }),
      "utf-8",
    );
  ...
  it.runIf(process.platform !== "win32" && typeof process.getuid === "function")(
    ...
    async () => {
      ...
      const result = await withStateDir(stateDir, async () => {
        return discoverAnvikaPlugins({ ownershipUid: actualUid + 1 });
      });
      expect(result.candidates).toHaveLength(0);
