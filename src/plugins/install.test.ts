...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `anvika-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        anvika: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function createZipperArchiveBuffer(): Promise<Buffer> {
  ...
  zip.file(
    ...
    JSON.stringify({
      name: "@anvika/zipper",
      version: "0.0.1",
      anvika: { extensions: ["./dist/index.js"] },
    }),
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.anvika/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without anvika.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@anvika/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("anvika.extensions");
  });
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.anvika).toBeUndefined();
    expect(manifest.devDependencies?.vitest).toBe("^3.0.0");
  });

  it("uses anvika.plugin.json id as install key when it differs from package name", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@anvika/cognee-anvika",
        version: "0.0.1",
        anvika: { extensions: ["./dist/index.js"] },
      }),
      "utf-8",
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "cognee-anvika"',
        ),
      ),
    ).toBe(true);
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@anvika/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@anvika/voice-call@0.0.1",
    });
