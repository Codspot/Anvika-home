...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("anvika-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "anvika-sandbox-browser-global" },
      agentBrowser: { network: "anvika-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("anvika-sandbox-browser-agent");
  });
