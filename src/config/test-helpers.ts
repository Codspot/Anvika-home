...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "anvika-config-" });
}

export async function writeAnvikaConfig(home: string, config: unknown): Promise<string> {
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeAnvikaConfig(home, config);
