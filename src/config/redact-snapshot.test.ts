...
import type { ConfigFileSnapshot } from "./types.anvika.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.anvika/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/anvika/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/anvika/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/anvika/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/anvika/nickserv-password.txt");
  ...
  it("contract-covers dynamic catchall/record paths for redact+restore", () => {
    const hints = mapSensitivePaths(AnvikaSchema, "", {});
    const snapshot = makeSnapshot({
      env: {
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = AnvikaSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "AnvikaConfig";
    const hints = mapSensitivePaths(AnvikaSchema, "", {});
