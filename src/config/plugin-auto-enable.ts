...
import type { AnvikaConfig } from "./config.js";
...
function isWhatsAppConfigured(cfg: AnvikaConfig): boolean {
...
function isGenericChannelConfigured(cfg: AnvikaConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: AnvikaConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: AnvikaConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: AnvikaConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: AnvikaConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: AnvikaConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: AnvikaConfig, pluginId: string): AnvikaConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
