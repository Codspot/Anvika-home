...
import type { AnvikaConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: AnvikaConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: AnvikaConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AnvikaSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AnvikaConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AnvikaConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as AnvikaConfig,
  };
}
