...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ANVIKA_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-anvika",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-anvika#general");
  });
  ...
  it("derives session transcripts dir from ANVIKA_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ANVIKA_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const stateDir = path.resolve("/home/user/.anvika");
  ...
  it("resolves cross-agent paths when ANVIKA_STATE_DIR differs from stored paths", () => {
  ...
  it("falls back to derived transcript path when sessionFile is outside agent sessions directories", () => {
    withStateDir(path.resolve("/home/user/.anvika"), () => {
      const sessionFile = resolveSessionFilePath(
        "sess-1",
