...
import type { AnvikaConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareAnvikaVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  anvika config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  anvika config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: AnvikaConfig,
  pathSegments: string[],
): { changed: boolean; next: AnvikaConfig } {
...
function coerceConfig(value: unknown): AnvikaConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as AnvikaConfig;
}
...
function stampConfigVersion(cfg: AnvikaConfig): AnvikaConfig {
...
function warnIfConfigFromFuture(cfg: AnvikaConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareAnvikaVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Anvika (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as AnvikaConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AnvikaConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as AnvikaConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: AnvikaConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as AnvikaConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.ANVIKA_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.ANVIKA_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.ANVIKA_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.ANVIKA_WATCH_SESSION === "string" &&
        deps.env.ANVIKA_WATCH_SESSION.trim().length > 0
          ? deps.env.ANVIKA_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.ANVIKA_WATCH_COMMAND === "string" &&
        deps.env.ANVIKA_WATCH_COMMAND.trim().length > 0
          ? deps.env.ANVIKA_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `ANVIKA_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: AnvikaConfig | null = null;
let runtimeConfigSourceSnapshot: AnvikaConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ANVIKA_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.ANVIKA_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): AnvikaConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): AnvikaConfig {
