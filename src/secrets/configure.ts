...
function getSecretProviders(config: AnvikaConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: AnvikaConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function buildCandidates(config: AnvikaConfig): ConfigureCandidate[] {
...
function toSourceChoices(config: AnvikaConfig): Array<{ value: SecretRefSource; label: string }> {
...
async function configureProvidersInteractive(config: AnvikaConfig): Promise<void> {
  while (true) {
...
function collectProviderPlanChanges(params: { original: AnvikaConfig; next: AnvikaConfig }): {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    if (candidates.length === 0) {
      throw new Error("No configurable secret-bearing fields found in anvika.json.");
    }
  ...
  const plan: SecretsApplyPlan = {
    ...
    generatedBy: "anvika secrets configure",
