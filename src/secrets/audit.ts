...
import { resolveStateDir, type AnvikaConfig } from "../config/config.js";
...
function collectAuthStorePaths(config: AnvikaConfig, stateDir: string): string[] {
...
export async function runSecretsAudit(
  ...
  const previousAuthStoreReadOnly = process.env.ANVIKA_AUTH_STORE_READONLY;
  process.env.ANVIKA_AUTH_STORE_READONLY = "1";
  try {
    ...
    const config = snapshot.valid ? snapshot.config : ({} as AnvikaConfig);
    ...
    if (previousAuthStoreReadOnly === undefined) {
      delete process.env.ANVIKA_AUTH_STORE_READONLY;
    } else {
      process.env.ANVIKA_AUTH_STORE_READONLY = previousAuthStoreReadOnly;
    }
  }
}
