...
describe("resolveGatewayConnection", () => {
  ...
  it("uses ANVIKA_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ ANVIKA_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers ANVIKA_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ ANVIKA_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
