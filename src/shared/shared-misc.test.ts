...
describe("shared/frontmatter", () => {
  ...
  test("resolveAnvikaManifestBlock parses JSON5 metadata and picks anvika block", () => {
    const frontmatter = {
      metadata: "{ anvika: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveAnvikaManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveAnvikaManifestBlock returns undefined for invalid input", () => {
    expect(resolveAnvikaManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveAnvikaManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveAnvikaManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
