...
type AgentEntry = NonNullable<NonNullable<AnvikaConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: AnvikaConfig): AgentEntry[] {
...
export function listAgentIds(cfg: AnvikaConfig): string[] {
...
export function resolveDefaultAgentId(cfg: AnvikaConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: AnvikaConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: AnvikaConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: AnvikaConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: AnvikaConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
