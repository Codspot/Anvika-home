...
function buildCoreToolGroupMap() {
  ...
  const anvikaTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInAnvikaGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:anvika": anvikaTools,
