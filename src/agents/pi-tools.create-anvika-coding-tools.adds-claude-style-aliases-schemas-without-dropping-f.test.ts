...
describe("createAnvikaCodingTools", () => {
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-alias-"));
    try {
      const tools = createAnvikaCodingTools({ workspaceDir: tmpDir });
  ...
  it("coerces structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-structured-write-"));
    try {
      const tools = createAnvikaCodingTools({ workspaceDir: tmpDir });
  ...
  it("coerces structured old/new text blocks for edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-structured-edit-"));
    try {
