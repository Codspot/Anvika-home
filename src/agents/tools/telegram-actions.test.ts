...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): AnvikaConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as AnvikaConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): AnvikaConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as AnvikaConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: AnvikaConfig, accountId = "media") {
    await handleTelegramAction(
