...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "anvika" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [
              { title: "Anvika", url: "https://anvika.ai/docs", content: "docs" },
            ],
          }),
    ...
    const result = await tool?.execute?.("call-1", { query: "latest anvika release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://anvika.ai/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://anvika.ai/docs"]);
