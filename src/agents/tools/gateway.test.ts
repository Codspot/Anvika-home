...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    anvika: process.env.ANVIKA_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.anvika === undefined) {
      delete process.env.ANVIKA_GATEWAY_TOKEN;
    } else {
      process.env.ANVIKA_GATEWAY_TOKEN = envSnapshot.anvika;
    }
  ...
  it("uses ANVIKA_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.ANVIKA_GATEWAY_TOKEN = "local-env-token";
