...
const GOOGLE_LIVE_SINGLE_KEY = "ANVIKA_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ANVIKA_LIVE_ANTHROPIC_KEY",
    listVar: "ANVIKA_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ANVIKA_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ANVIKA_LIVE_${base}_KEY`;
