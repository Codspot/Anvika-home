...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.ANVIKA_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".anvika", `workspace-${profile}`);
  }
  return path.join(home, ".anvika", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".anvika";
