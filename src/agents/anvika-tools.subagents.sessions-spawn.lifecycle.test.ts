...
} from "./anvika-tools.subagents.sessions-spawn.test-harness.js";
import { resetSubagentRegistryForTests } from "./subagent-registry.js";

const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.ANVIKA_TEST_FAST;
...
describe("anvika-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
    setSessionsSpawnConfigOverride({
      session: {
  ...
  afterAll(() => {
    if (fastModeEnv.previous === undefined) {
      delete process.env.ANVIKA_TEST_FAST;
      return;
    }
    process.env.ANVIKA_TEST_FAST = fastModeEnv.previous;
  });
