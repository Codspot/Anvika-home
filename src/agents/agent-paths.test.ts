...
describe("resolveAnvikaAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-agent-"));
    try {
      run(stateDir);
    } finally {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    await withTempStateDir((stateDir) => {
      withEnv(
        ...
        () => {
          const resolved = resolveAnvikaAgentDir();
  ...
  it("honors ANVIKA_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          ANVIKA_STATE_DIR: undefined,
          ANVIKA_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveAnvikaAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when ANVIKA_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers ANVIKA_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          ANVIKA_STATE_DIR: undefined,
          ANVIKA_AGENT_DIR: primaryOverride,
