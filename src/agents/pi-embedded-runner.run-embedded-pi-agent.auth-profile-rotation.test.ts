...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensureAnvikaModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): AnvikaConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): AnvikaConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-workspace-"));
    const now = Date.now();
