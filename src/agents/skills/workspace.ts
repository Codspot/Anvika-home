...
function resolveSkillsLimits(config?: AnvikaConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveAnvikaMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: AnvikaConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`anvika skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
