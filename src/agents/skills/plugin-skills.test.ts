    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "anvika.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "anvika.plugin.json"),
      },
    ],
  };
}
...
describe("resolvePluginSkillDirs", () => {
  it("keeps acpx plugin skills when ACP is enabled", async () => {
    const workspaceDir = await tempDirs.make("anvika-");
    const acpxRoot = await tempDirs.make("anvika-acpx-plugin-");
    const helperRoot = await tempDirs.make("anvika-helper-plugin-");
