...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ANVIKA_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns ANVIKA_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-bundled-override-"));
    process.env.ANVIKA_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.ANVIKA_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "anvika" }));
