...
import { createAnvikaCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: AnvikaConfig) {
    return createAnvikaCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<AnvikaConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<AnvikaConfig["agents"]>["list"]>[number]["tools"];
  }): AnvikaConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createAnvikaCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createAnvikaCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createAnvikaCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createAnvikaCodingTools({
      config: cfg,
    ...
    const bobTools = createAnvikaCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createAnvikaCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createAnvikaCodingTools({
      config: cfg,
