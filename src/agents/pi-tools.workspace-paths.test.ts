...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("anvika-ws-", async (workspaceDir) => {
      await withTempDir("anvika-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createAnvikaCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("anvika-ws-", async (workspaceDir) => {
      await withTempDir("anvika-override-", async (overrideDir) => {
        const tools = createAnvikaCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("anvika-ws-", async (workspaceDir) => {
      const cfg: AnvikaConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createAnvikaCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-anvika.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("anvika-sandbox-", async (sandboxDir) => {
      await withTempDir("anvika-workspace-", async (workspaceDir) => {
        ...
        const tools = createAnvikaCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
