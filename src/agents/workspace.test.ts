...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ANVIKA_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ANVIKA_HOME: "/srv/anvika-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/anvika-home"), ".anvika", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".anvika", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("anvika-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-workspace-hardlink-"));
    try {
