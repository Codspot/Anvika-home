...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "anvika.sandbox=1");
  args.push("--label", `anvika.sessionKey=${params.scopeKey}`);
  args.push("--label", `anvika.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `anvika.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "anvika.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`anvika sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`anvika sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("anvika sandbox recreate --all");
}
