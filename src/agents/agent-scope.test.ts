...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: AnvikaConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: AnvikaConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: AnvikaConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: AnvikaConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: AnvikaConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: AnvikaConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/anvika");
  });

  it("uses ANVIKA_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "anvika-home");
    vi.stubEnv("ANVIKA_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as AnvikaConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".anvika", "workspace"));
  });

  it("uses ANVIKA_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "anvika-home");
    vi.stubEnv("ANVIKA_HOME", home);
    // Clear state dir so it falls back to ANVIKA_HOME
    vi.stubEnv("ANVIKA_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as AnvikaConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".anvika", "agents", "main", "agent"));
  });
});
