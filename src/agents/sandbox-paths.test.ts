...
import { resolvePreferredAnvikaTmpDir } from "../infra/tmp-anvika-dir.js";
...
async function withOutsideHardlinkInAnvikaTmp<T>(
  params: {
    anvikaTmpDir: string;
  ...
  const hardlinkPath = path.join(params.anvikaTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.anvikaTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.anvikaTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.anvikaTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const anvikaTmpDir = resolvePreferredAnvikaTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Anvika tmp root",
      media: path.join(anvikaTmpDir, "image.png"),
      expected: path.join(anvikaTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Anvika tmp root",
      media: pathToFileURL(path.join(anvikaTmpDir, "photo.png")).href,
      expected: path.join(anvikaTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Anvika tmp root",
      media: path.join(anvikaTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(anvikaTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(anvikaTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside anvika tmp root",
      media: path.join(os.tmpdir(), "outside-anvika", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Anvika tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(anvikaTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(anvikaTmpDir, { recursive: true });
      const symlinkPath = path.join(anvikaTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Anvika tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInAnvikaTmp(
      {
        anvikaTmpDir,
  ...
  it("rejects symlinked Anvika tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInAnvikaTmp(
