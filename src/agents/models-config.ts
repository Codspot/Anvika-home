...
type ModelsConfig = NonNullable<AnvikaConfig["models"]>;
...
export async function ensureAnvikaModelsJson(
  config?: AnvikaConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveAnvikaAgentDir();
