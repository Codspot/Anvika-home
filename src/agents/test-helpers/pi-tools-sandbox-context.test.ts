...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("anvika-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("anvika-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("anvika-sbx-");
