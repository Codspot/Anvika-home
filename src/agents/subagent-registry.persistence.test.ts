...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["ANVIKA_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "anvika-subagent-"));
  ...
  it("uses isolated temp state when ANVIKA_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "anvika-test-state"));
  });
});
