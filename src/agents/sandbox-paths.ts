...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const anvikaTmpDir = path.resolve(resolvePreferredAnvikaTmpDir());
  if (!isPathInside(anvikaTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: anvikaTmpDir });
  return resolved;
}
