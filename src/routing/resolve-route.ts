...
function listAgents(cfg: AnvikaConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: AnvikaConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: AnvikaConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<AnvikaConfig, EvaluatedBindingsCache>();
