...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "anvika.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "anvika.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.ANVIKA_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.ANVIKA_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.ANVIKA_NODE_OPTIONS_READY = "1";
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[anvika] ${parsed.error}`);
      process.exit(2);
    }
