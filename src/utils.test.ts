...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("anvika-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("anvika-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("anvika-lid-a-", (first) => {
      withTempDirSync("anvika-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.anvika when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "anvika-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers ANVIKA_HOME over HOME", () => {
    vi.stubEnv("ANVIKA_HOME", "/srv/anvika-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/anvika-home"));
...
describe("shortenHomePath", () => {
  it("uses $ANVIKA_HOME prefix when ANVIKA_HOME is set", () => {
    vi.stubEnv("ANVIKA_HOME", "/srv/anvika-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/anvika-home")}/.anvika/anvika.json`)).toBe(
      "$ANVIKA_HOME/.anvika/anvika.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $ANVIKA_HOME replacement when ANVIKA_HOME is set", () => {
    vi.stubEnv("ANVIKA_HOME", "/srv/anvika-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/anvika-home")}/.anvika/anvika.json`),
    ).toBe("config: $ANVIKA_HOME/.anvika/anvika.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/anvika")).toBe(path.resolve(os.homedir(), "anvika"));
  });
  ...
  it("prefers ANVIKA_HOME for tilde expansion", () => {
    vi.stubEnv("ANVIKA_HOME", "/srv/anvika-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/anvika")).toBe(path.resolve("/srv/anvika-home", "anvika"));
