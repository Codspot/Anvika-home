...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".anvika");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ANVIKA_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ANVIKA_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via ANVIKA_STATE_DIR.
