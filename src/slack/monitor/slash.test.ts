...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "anvika_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "anvika_cmdarg", "arg-menu options");
  });
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(testHarness.actions.has("anvika_cmdarg")).toBe(true);
    expect(testHarness.options.has("anvika_cmdarg")).toBe(true);
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("anvika_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("anvika_cmdarg");
    expect(blockId).toContain("anvika_cmdarg_ext:");
    const token = (blockId ?? "").slice("anvika_cmdarg_ext:".length);
